...
// Predefined option sets for dropdowns
...
function refreshLeftPanel() {
    ...
    console.log('[BlueprintEditor] refreshLeftPanel called, leftPanel found:', leftPanel.length > 0);
    if (leftPanel.length) {
        leftPanel.html(renderLeftPanel());
        console.log('[BlueprintEditor] Left panel refreshed, new story_type_name:', currentBlueprint.story_type_name);
    } else {
        console.error('[BlueprintEditor] Could not find left panel to refresh');
    }
}
                ...
                <div class="storymode-cover-carousel-column">
                    ...
                    <div class="storymode-cover-prompt-editor">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 12px;">
                            ...
                            <div>
                                <label style="font-size: 0.8em; color: var(--SmartThemeEmColor);">Style</label>
                                <input type="text" id="cover_style" class="text_pole" data-cover-field="style"
                                    value="${escapeHtml(coverPrompt.style || '')}" placeholder="e.g., digital painting"
                                    style="width: 100%; font-size: 0.85em;">
                            </div>
                            <div>
                                <label style="font-size: 0.8em; color: var(--SmartThemeEmColor);">Mood</label>
                                <input type="text" id="cover_mood" class="text_pole" data-cover-field="mood"
                                    value="${escapeHtml(coverPrompt.mood || '')}" placeholder="e.g., mysterious"
                                    style="width: 100%; font-size: 0.85em;">
                            </div>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                                <div>
                                    <label style="font-size: 0.8em; color: var(--SmartThemeEmColor);">Aspect</label>
                                    <select id="cover_aspect_ratio" class="text_pole" data-cover-field="technical.aspect_ratio" style="width: 100%; font-size: 0.85em;">
                                        <option value="2:3" ${coverPrompt.technical?.aspect_ratio === '2:3' ? 'selected' : ''}>2:3</option>
                                        <option value="3:4" ${coverPrompt.technical?.aspect_ratio === '3:4' ? 'selected' : ''}>3:4</option>
                                        <option value="1:1" ${coverPrompt.technical?.aspect_ratio === '1:1' ? 'selected' : ''}>1:1</option>
                                        <option value="16:9" ${coverPrompt.technical?.aspect_ratio === '16:9' ? 'selected' : ''}>16:9</option>
                                    </select>
                                </div>
                                <div>
                                    <label style="font-size: 0.8em; color: var(--SmartThemeEmColor);">Quality</label>
                                    <select id="cover_quality" class="text_pole" data-cover-field="technical.quality" style="width: 100%; font-size: 0.85em;">
                                        <option value="draft" ${coverPrompt.technical?.quality === 'draft' ? 'selected' : ''}>Draft</option>
                                        <option value="standard" ${coverPrompt.technical?.quality === 'standard' ? 'selected' : ''}>Standard</option>
                                        <option value="high" ${coverPrompt.technical?.quality === 'high' ? 'selected' : ''}>High</option>
                                    </select>
                                </div>
                            </div>
...
function buildSDPrompt(coverPrompt) {
    ...
    const negative = coverPrompt.negative;

    // SD slash command format: "positive --no negative"
    return negative ? `${positive} --no ${negative}` : positive;
}
// ============================================================================
 ...
 */
async function addCoverToGallery(blueprint, imageUrl, prompt, metadata = {}) {
...
async function handleGenerateCover(e) {
    ...
    try {
        const prompt = currentBlueprint.metadata?.coverPrompt;
        if (!prompt) throw new Error('No cover prompt defined');
        
        // ✅ GENERIC FIX: Ensure SD extension settings are loaded (any provider)
        if (!extension_settings.sd || !extension_settings.sd.source) {
            ...
            // If still not initialized, fail with helpful message
            if (!extension_settings.sd || !extension_settings.sd.source) {
                throw new Error(
                    'SD extension not properly initialized. ' +
                    'Please configure image generation in SD settings or run /sd manually first.'
                );
            }
        }
        
        // Log current provider for debugging (not validation - just info)
        console.log('[BlueprintEditor] Image Provider:', extension_settings.sd.source);
        
        // Provider-specific debug info (optional, non-blocking)
        switch (extension_settings.sd.source) {
            case 'comfy':
                console.log('[BlueprintEditor] ComfyUI URL:', extension_settings.sd.comfyurl);
                break;
            case 'auto':
            case 'vlad':
                console.log('[BlueprintEditor] WebUI URL:', extension_settings.sd.autourl || extension_settings.sd.vladurl);
                break;
            case 'novel':
                console.log('[BlueprintEditor] Using NovelAI');
                break;
            case 'openai':
                console.log('[BlueprintEditor] Using OpenAI DALL-E');
                break;
            case 'horde':
                console.log('[BlueprintEditor] Using Stable Horde');
                break;
            default:
                console.log('[BlueprintEditor] Using provider:', extension_settings.sd.source);
        }
        
        const fullPrompt = buildSDPrompt(prompt);
        console.log('[BlueprintEditor] Generating cover with prompt:', fullPrompt);
        
        // The SD extension handles provider-specific logic internally
        ...
        if (typeof result === 'string' && result.trim().length > 0) {
            ...
            await addCoverToGallery(currentBlueprint, imageUrl, prompt);
            
            currentBlueprint.metadata = currentBlueprint.metadata || {};
            currentBlueprint.metadata.coverImageUrl = imageUrl;
            currentBlueprint.coverImageUrl = imageUrl;
        ...
        const currentBtn = $('#generate_cover_btn');
...
async function handlePlayButtonClick(e) {
    ...
    const result = await BlueprintModule.startStoryFromBlueprint(currentBlueprint);
