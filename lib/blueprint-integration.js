 ...
 * - blueprint-library.js (IndexedDB storage)
...
import { getLibrary } from './blueprint-library.js';
import { getMerger } from './blueprint-merger.js';
import {
...
import { openBlueprintEditor } from './blueprint-editor.js';
import {
    generateUUID,
...
export {
    ...
    BlueprintMerger,
    CharacterMerger,
} from './blueprint-merger.js';

// Re-export editor function
export { openBlueprintEditor } from './blueprint-editor.js';
...
export async function importBlueprintFromPNG(file, options = {}) {
    ...
    if (saveToLibrary) {
        const library = await getLibraryCached();
        await library.importer.importFromFile(file);
        invalidateLibraryCache();
    }
...
export async function startStoryFromLibrary(blueprintId, options = {}) {
    ...
    // Generate opening message if requested
    if (generateOpeningMessage) {
        try {
            const { BlueprintModule } = await import('./blueprint-module.js');
            const opening = await BlueprintModule.generateOpeningMessage(blueprint);
            if (opening && window.pushStoryMessage) {
                await window.pushStoryMessage(opening);
            }
        } catch (error) {
            console.warn('[BlueprintIntegration] Opening message generation failed:', error);
        }
    }

    return { success: true, blueprint, mergeResult, openingGenerated: generateOpeningMessage };
}
...
export async function setBlueprintFavorite(blueprintId, favorite) {
    ...
    if (blueprint) {
        ...
        blueprint.userMetadata.favorite = favorite;
        await library.saveBlueprint(blueprint);
        invalidateLibraryCache();
    }
}
...
export async function saveCurrentBlueprintToLibrary(options = {}) {
    const { title = null, description = null, tags = [], generateCover = false } = options;

    const current = await getCurrentBlueprint();
    if (!current) {
...
export async function initBlueprintIntegration() {
    try {
        ...
        console.log('[BlueprintIntegration] Initialized successfully');
