...
export const PHASE_CONFIG = {
    ...
    4: {
        ...
        maxTokens: 16384, // High limit required for reasoning models (o1, o3-mini, DeepSeek-R1)
    },
};
 ...
 * @returns {boolean} True if beat was newly marked completed
 */
 ...
 * @param {number} sceneIndex - Scene index
 * @param {number} beatIndex - Beat index within scene
 * @returns {boolean} True if beat was newly marked skipped
 */
 ...
 * @returns {string} The prompt injection XML
 */
function buildScenarioModeInjection(blueprintState) {
    const beatState = getBeatState();
    ...
    if (scene.character_focus && scene.character_focus.length > 0) {
        const resolved = scene.character_focus.map(c => resolvePlaceholders(c, blueprint));
        characterFocusXml = `\n  <focus_characters>${resolved.join(', ')}</focus_characters>`;
    }
 ...
 */
function deriveExitTrigger(purpose, isFinalScene) {
    if (!purpose) {
        return isFinalScene
 ...
 */
function buildSignalsBlock(activeBeatIndex, isFinalScene) {
 ...
 * Replaces [Role] tokens with character names from the blueprint.
 *
 * Supported placeholders:
 * - [Protagonist] → First character from character_arcs
 * - [Antagonist] → Extracted from antagonistic_forces if named, else "the antagonist"
 * - [Guide] / [Mentor] → Character with mentor-like role if found
 * - [Character:N] → Nth character from character_arcs (0-indexed)
 *
...
export function checkPrerequisites(beat, scenarioState) {
    ...
    // If beat has 'requires' field, check it (not yet in schema, but good for future)
    return true;
}

/**
 * Build the blueprint injection for the current scene.
 ...
 */
async function buildCharacterSelectionList() {
    const { getCurrentChatCharacters } = await import('./blueprint-character-linker.js');
 ...
 * @returns {string} HTML string
 */
async function buildPersonaSelectionList() {
    const { getAllPersonas } = await import('./blueprint-character-linker.js');
