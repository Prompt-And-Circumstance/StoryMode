...
import { escapeHtml, blobToDataURL, dataURLtoBlob, dataURLtoFile, normalizeCharacterName } from '../utils.js';
...
export async function importCharacterCard(pngDataUrl, characterName) {
    try {
        ...
        // verifyPNGSignature expects Uint8Array (not ArrayBuffer) and throws on failure
        const arrayBuffer = await blob.arrayBuffer();
        verifyPNGSignature(new Uint8Array(arrayBuffer));
        ...
        // Upload to server
        // omitContentType lets the browser set multipart/form-data with boundary
...
/**
 * Link blueprint character_arcs to local characters and personas
 * @param {Object} blueprint - Blueprint with character_arcs
 * @returns {Object} {linked: [], linkedPersonas: [], missing: []}
 */
export function linkBlueprintCharacters(blueprint) {
    const linked = [];
    const linkedPersonas = [];
    const missing = [];

    if (!blueprint.character_arcs || blueprint.character_arcs.length === 0) {
        return { linked, linkedPersonas, missing };
    }
    ...
    for (const arc of blueprint.character_arcs) {
        ...
        // Then check personas
        const localPersona = allPersonas.find(p =>
            normalizeCharacterName(p.name) === normalizeCharacterName(charName)
        );
