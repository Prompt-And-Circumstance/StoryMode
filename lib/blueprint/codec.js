 ...
 * Blueprint PNG Codec
 * 
 * Handles encoding and decoding of Story Mode blueprints to/from PNG files.
 * This module is pure and dependency-free (except for utils), making it safe
 * for use in the Standalone Editor.
 *
 * @module blueprint/codec
 */
...
/**
 * Initialize cover gallery from existing cover (legacy migration)
 * @param {Object} blueprint - Blueprint object
 */
function initGalleryFromExistingCover(blueprint) {
...
/**
 * Initialize cover gallery with placeholder SVG
 * @param {Object} blueprint - Blueprint object
 */
async function initGalleryWithPlaceholder(blueprint) {
    const { generatePlaceholderCover } = await import('./blank-blueprint.js');
    const title = blueprint.userMetadata?.title ||
        blueprint.blueprint_title ||
        blueprint.core_premise?.substring(0, 30) ||
...
async function ensureBlueprintMetadata(blueprint) {
    ...
    // Initialize cover gallery if missing
    if (!blueprint.metadata.coverGallery?.length) {
        const existingCover = blueprint.coverImageUrl || blueprint.metadata.coverImageUrl;
        if (existingCover) {
            initGalleryFromExistingCover(blueprint);
        } else {
            await initGalleryWithPlaceholder(blueprint);
        }
    }
}
