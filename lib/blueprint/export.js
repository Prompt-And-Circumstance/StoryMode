...
import { resolveStoryType, resolveAuthorStyle, buildAuthorStyleFromInlineFields } from './resource-utils.js';
import { sanitizeFilename, downloadBlob, estimateDataURLSize, formatBytes, getBlueprintCoverUrl, isDelinked } from './utils.js';
...
async function estimateExportSize(blueprint, options) {
    ...
    if (options.includeCharacters && blueprint.character_arcs) {
        const characterNames = getBlueprintCharacterNames(blueprint)
            .filter(name => !isDelinked(blueprint, name));
...
async function gatherEmbeddedResources(blueprint, options) {
    ...
    if (blueprint.selectedPersonas || blueprint.personaData) {
        const personaSource = blueprint.selectedPersonas || blueprint.personaData || [];
        const personaNames = personaSource
            .map(p => typeof p === 'string' ? p : p?.name)
            .filter(name => name && !isDelinked(blueprint, name));
        if (personaNames.length > 0) {
            enriched.embeddedResources.personas = await extractPersonasForExport(personaNames);
        }
    }
    ...
    // Story type - three-tier fallback (library → inline → previous embedded)
    if (options.includeStoryType) {
        const resolved = resolveStoryType(blueprint, getStoryTypes());
        if (resolved) enriched.embeddedResources.storyType = resolved;
    }

    // Author style - three-tier fallback, plus no-ID inline path
    if (options.includeAuthorStyle) {
        const resolved = resolveAuthorStyle(blueprint, getAuthorStyles());
        if (resolved) {
            enriched.embeddedResources.authorStyle = resolved;
        } else if (blueprint.author_style_name && blueprint.author_style_prompt) {
            // No ID reference but has inline data — generate an ID for the export
            const inlineId = `inline_${Date.now()}`;
            enriched.embeddedResources.authorStyle = buildAuthorStyleFromInlineFields(blueprint, inlineId);
            enriched.author_style = inlineId;
        }
    }

    // Remove delinkedCharacters from export - it's only for editor state
    delete enriched.delinkedCharacters;
...
function generateExportFilename(blueprint) {
    // Prefer blueprint_title (scenario title), fall back to story_type_name, then default
    const baseName = blueprint.blueprint_title || blueprint.story_type_name || 'blueprint';
