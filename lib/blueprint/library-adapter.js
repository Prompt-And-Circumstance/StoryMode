...
import { loadPlayStatsFromFile, savePlayStatsToFile } from '../core/file-backed-data.js';
...
/** In-memory cache for play stats (loaded from File API at init) */
let cachedPlayStats = null;
...
function loadPlayStats() {
    return cachedPlayStats || {};
}

function savePlayStats(stats) {
    cachedPlayStats = stats;
    savePlayStatsToFile(stats);
}
...
export class FileBackedLibrary {
    ...
    async init() {
        ...
        cachedPlayStats = await loadPlayStatsFromFile();
    ...
    async saveBlueprint(blueprint) {
        ...
        console.log('[LibraryAdapter] saveBlueprint called with ID:', blueprint.blueprint_id);
        console.log('[LibraryAdapter] Blueprint title:', blueprint.userMetadata?.title || blueprint.blueprint_title);
        ...
        console.log('[LibraryAdapter] Calling saveBlueprintToFile with ID:', blueprint.blueprint_id);
        await saveBlueprintToFile(blueprint, coverCanvas);
        console.log('[LibraryAdapter] saveBlueprintToFile completed');
    }
