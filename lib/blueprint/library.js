...
import { generateUUID, downloadBlob, loadImage, fileToDataURL } from './utils.js';
...
function openDatabase() {
    return new Promise((resolve, reject) => {
        ...
        if (!window.indexedDB) {
            console.error('[BlueprintLibrary] IndexedDB not available in this browser');
            reject(new Error('IndexedDB not available. Check browser privacy settings or disable private browsing mode.'));
            return;
        }
        ...
        request.onerror = (event) => {
            ...
            console.error('[BlueprintLibrary] Database open failed:', {
                name: error?.name,
                message: error?.message,
                code: error?.code
            });
            reject(new Error(`Failed to open database: ${error?.message || 'Unknown error'}. Check browser privacy/shield settings.`));
        };
        request.onsuccess = () => {
            console.log('[BlueprintLibrary] Database opened successfully');
...
export class BlueprintImporter {
    ...
    async importFromFile(file) {
        const { isBlueprintPNG, decodeBlueprintFromPNG, extractMetadataFromPNG } = await import('./storage.js');
    ...
    async importFromExtendedPNG(file) {
        const { importBlueprintFromPNG } = await import('./import.js');
