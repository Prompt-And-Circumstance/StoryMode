    ...
    for (let i = 0; i < blueprints.length; i++) {
        ...
        try {
            ...
            if (hasEntry(blueprint.blueprint_id)) {
                console.log(`[Migration] Skipping already migrated: ${blueprint.blueprint_id}`);
                result.migrated++;
                continue;
            }
...
export async function promptAndMigrate() {
    ...
    const proceed = confirm(
        `Story Mode has ${count} blueprint(s) in browser storage that need to be migrated to server storage.\n\n` +
        `This one-time migration will:\n` +
        `• Copy your blueprints to the server\n` +
        `• Make them accessible across browsers\n` +
        `• Preserve all your data\n\n` +
        `Click OK to migrate now, or Cancel to skip (you can migrate later from Settings).`
    );
