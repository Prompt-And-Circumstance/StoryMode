...
import { validateOpeningMessage, resolveBeatType } from './utils.js';
...
export function normalizeBlueprint(blueprint) {
    ...
    if (Object.prototype.hasOwnProperty.call(blueprint, 'author_style_nsfw_prompt')) {
        normalized.author_style_nsfw_prompt = blueprint.author_style_nsfw_prompt;
    }

    // Preserve inline story type fields if present
    for (const field of [
        'story_type_prompt',
        'story_type_category',
        'story_type_progress_template',
        'story_type_phase_prompts',
        'story_type_memorable_element',
    ]) {
        if (Object.prototype.hasOwnProperty.call(blueprint, field)) {
            normalized[field] = blueprint[field];
        }
    }
    ...
    normalized.scene_plan = (normalized.scene_plan || []).map((scene, index) => {
        ...
        if (scene.beats && Array.isArray(scene.beats)) {
            normalizedScene.beats = scene.beats.map((beat, beatIndex) => {
                const originalType = beat.type;
                const resolvedType = resolveBeatType(originalType);
                ...
                return {
                    ...
                    type: resolvedType,
                    original_type: (originalType && originalType !== resolvedType) ? originalType : undefined,
