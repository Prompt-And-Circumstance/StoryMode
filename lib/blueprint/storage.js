...
async function buildMetadataChunks(blueprint) {
    // Create a copy of blueprint to sanitize for storage
    const storageBlueprint = { ...blueprint };

    // Remove transient blob URLs from top-level properties
    if (storageBlueprint.coverImageUrl && storageBlueprint.coverImageUrl.startsWith('blob:')) {
...
// ============================================================================
// BLUEPRINT PNG UPDATE (Metadata only)
// ============================================================================

/**
 * Update blueprint metadata in an existing PNG without re-encoding image data
 * @param {Uint8Array} originalPngBytes - Original PNG bytes
 * @param {Object} updatedBlueprint - Updated blueprint object (with new ID/Title)
 * @returns {Promise<Blob>} New PNG blob with updated metadata
 */
export async function updateBlueprintMetadataInPNG(originalPngBytes, updatedBlueprint) {
    ensureBlueprintMetadata(updatedBlueprint);

    // Generate new metadata chunks
    const chunks = await buildMetadataChunks(updatedBlueprint);

    // Replace existing metadata chunks to avoid old values overwriting new ones during decode
    const newBytes = replaceMetadataChunks(originalPngBytes, chunks);
...
async function decodeInternal(bytes) {
    ...
    // Create data URL for cover image (not blob URL - blob URLs don't survive page reload)
    // Use chunked approach to avoid call stack issues with large images
    ...
    blueprint.coverImageUrl = `data:image/png;base64,${btoa(binaryStr)}`;
