...
async function coverToCanvas(coverImage, blueprint) {
    ...
    // Check cover gallery first (contains persistent data: URLs)
    ...
    if (Array.isArray(gallery) && gallery.length > 0) {
        ...
        if (galleryItem?.url && galleryItem.url.startsWith('data:')) {
            try {
                console.log('[BlueprintStorage] Using cover from gallery index', selectedIndex);
    ...
    if (typeof blueprintCover === 'string') {
        // Handle data: URLs (new covers) and blob: URLs (loaded from file)
        if (blueprintCover.startsWith('data:') || blueprintCover.startsWith('blob:')) {
            try {
