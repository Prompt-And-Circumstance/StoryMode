            ...
            if (scene.beats) {
                if (!Array.isArray(scene.beats)) {
                    ...
                    scene.beats.forEach((beat, beatIndex) => {
                        ...
                        if (beat.type && !['establishment', 'hook', 'reaction', 'escalation', 'pivot', 'emotional', 'transition', 'action', 'dialogue', 'discovery', 'conflict', 'resolution', 'climax'].includes(beat.type)) {
                            errors.push(`scene_plan[${index}].beats[${beatIndex}].type must be one of: establishment, hook, reaction, escalation, pivot, emotional, transition, action, dialogue, discovery, conflict, resolution, climax`);
                        }
                    });
                }
            }
