            ...
            if (scene.beats && scene.beats.length > 0) {
                ...
                const beatMarkers = scene.beats.map((beat, idx) => {
                    if (pacingMode === 'scenario') {
                        // Scenario Mode: Use detailed state
                        const state = scenarioState.beatState[idx]?.status || 'pending';
                        ...
                        // Check if it's the suggested next beat (simple check: first pending)
                        // This replicates the logic in prompt generation
                        const isNext = !['complete', 'skipped'].includes(state) &&
                            scene.beats.slice(0, idx).every(b => {
                                const s = scenarioState.beatState[scene.beats.indexOf(b)]?.status;
