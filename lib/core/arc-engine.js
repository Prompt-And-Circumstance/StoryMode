...
export function buildFullInjection(isPreview = false) {
    ...
    // Check if blueprint is active (used for Scenario Mode injection)
    ...
    // In Scenario Mode, continue past arcLength since pacing is scene-based not round-based
    const pacingMode = getPacingMode();
    if (settings.storyArcEnabled && chatState.selectedStoryType) {
        ...
        const shouldBuildStory = chatState.currentStep < chatState.arcLength || isPreview || pacingMode === 'scenario';
        if (storyType && shouldBuildStory) {
            ...
            // Add phase guidance (minimal in Scenario Mode, full in Story Mode)
    ...
    // Author style injection - use effective author style (character > group > chat > default)
    ...
    // Blueprint injection - only in Scenario Mode (signal-based pacing)
    // Story Mode uses simpler story type + author style prompts instead
    if (blueprintActive && blueprintState && pacingMode === 'scenario') {
