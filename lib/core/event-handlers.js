...
async function handleUserMessageStep(data) {
    ...
    // Check if we should trigger arc completion (only in Story Mode, not Scenario Mode)
    // In Scenario Mode, arc completion is triggered by @@STORY_COMPLETE@@ signal only
    const pacingMode = getPacingMode();
    if (pacingMode !== 'scenario' && chatState.currentStep >= chatState.arcLength) {
        await handleArcCompletion(chatState, settings);
        return;
    }
...
async function handleAIMessageChecks(data) {
    ...
    // In Scenario Mode, arc completion is triggered by @@STORY_COMPLETE@@ signal only
    const pacingMode = getPacingMode();
    if (pacingMode !== 'scenario' && chatState.currentStep >= chatState.arcLength && !chatState.endNoticeShown) {
        await handleArcCompletion(chatState, settings);
    }
}
...
async function handleArcCompletion(chatState, settings) {
    ...
    if (settings.epilogueEnabled && !chatState.epilogueShown) {
        ...
        if (epilogue) {
            ...
            // Save content for Arc History viewer
            chatState.savedEpilogue = {
                text: epilogue,
    ...
    if (settings.summaryEnabled && !chatState.summaryShown) {
        ...
        if (!settings.epilogueEnabled || chatState.epilogueShown) {
            ...
            if (summary) {
                ...
                // Wait a moment for UI to settle before generating next adventure
                await new Promise(resolve => setTimeout(resolve, 1000));
            } else {
                console.warn('[Story Mode] Summary generation returned empty - marking as attempted to avoid blocking next adventure');
                // Mark as shown even though it failed, so we don't block next adventure/end notice
                chatState.summaryShown = true;
                await saveChatStoryState(chatState);
    ...
    // Generate and push next adventure if enabled and prerequisites are done
    if (settings.nextAdventureEnabled && !chatState.nextAdventureShown) {
        // Wait for epilogue and summary to complete (if enabled)
        const prerequisitesMet =
            (!settings.epilogueEnabled || chatState.epilogueShown) &&
            (!settings.summaryEnabled || chatState.summaryShown);

        if (prerequisitesMet) {
            console.log('[Story Mode] Generating next adventure...');
            const result = await generateNextAdventureForStory();
            if (result && result.scenarios?.length) {
                console.log('[Story Mode] Next adventure generated, pushing to chat');
                chatState.savedNextAdventure = {
                    themes: result.themes || [],
                    scenarios: result.scenarios,
                    timestamp: new Date().toISOString()
                };
                const messageText = formatNextAdventureMessage(result);
                await pushStoryMessage(messageText);
                chatState.nextAdventureShown = true;
                await saveChatStoryState(chatState);
                console.log('[Story Mode] Next adventure complete');
                // Wait a moment for UI to settle before showing end notice
                await new Promise(resolve => setTimeout(resolve, 1000));
            } else {
                console.warn('[Story Mode] Next adventure generation failed - marking as attempted');
                chatState.nextAdventureShown = true;
                await saveChatStoryState(chatState);
                if (toastr) toastr.warning('Next adventure generation failed. Continuing with arc completion.');
            }
        } else {
            console.log('[Story Mode] Waiting for epilogue/summary before generating next adventure');
        }
    } else {
        console.log('[Story Mode] Skipping next adventure:', settings.nextAdventureEnabled ? 'already shown' : 'disabled');
    }

    // Show end notice if conditions are met
    const conditionsMet =
        // A) None of the options are enabled
        (!settings.epilogueEnabled && !settings.summaryEnabled && !settings.nextAdventureEnabled)
        // B) All enabled options have been shown
        || (
            (!settings.epilogueEnabled || chatState.epilogueShown) &&
            (!settings.summaryEnabled || chatState.summaryShown) &&
            (!settings.nextAdventureEnabled || chatState.nextAdventureShown)
        );
...
async function generateNextAdventureForStory() {
    ...
    // Build context from recent messages and saved summary
    let contextText = '';
    ...
    console.log('[Story Mode] Generating next adventure with profile:', profileName);
    ...
    try {
        ...
        if (!responseText) {
            console.error('[Story Mode] Next adventure generation returned empty response');
            return null;
        }
        ...
        if (!jsonMatch) {
            console.error('[Story Mode] No JSON found in response');
            console.error('[Story Mode] Response text:', responseText.substring(0, 500));
            return null;
        }
        ...
        try {
            ...
            console.error('[Story Mode] Attempted to parse:', jsonMatch[0].substring(0, 500));
            return null;
        }
    } catch (error) {
        console.error('[Story Mode] Next adventure generation failed:', error);
        console.error('[Story Mode] Error details:', {
            profileId: selectedProfileId,
            errorMessage: error.message,
            errorStack: error.stack,
        });
