 ...
 * Format: { selector, line, event, description, delegated? }
 *
 * Entries with `delegated: true` already use event delegation via
 * `content.on(event, selector, ...)` and are only absent from the DOM
 * because they live in conditionally-rendered subtabs. The audit skips them.
 */
const DIRECT_BINDING_HANDLERS = [
    ...
    { selector: '#group_author_style_select', line: 452, event: 'change', description: 'Group author style dropdown', delegated: true },
    { selector: '#save_group_author_style_btn', line: 457, event: 'click', description: 'Save group author style button', delegated: true },
...
export async function auditHandlers(options = {}) {
    ...
    for (const handler of DIRECT_BINDING_HANDLERS) {
        // Skip entries already using event delegation — they work even
        // when the element doesn't exist yet (e.g. conditional subtabs)
        if (handler.delegated) {
            results.found.push({ ...handler, found: true, count: 0, delegated: true });
            continue;
        }
