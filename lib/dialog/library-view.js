...
// Removed: createRunCopy, saveBlueprintState - now handled by startStoryFromBlueprint
import {
    getLibrary,
    searchBlueprints,
...
/**
 * Apply favorites filter to currently displayed cards
 * Re-applies the filter state from the button without reloading data
 * @param {jQuery} content - The settings dialog content element
 */
export function applyFavoritesFilter(content) {
    const filterBtn = content.find('#library_favorites_filter');
    const icon = filterBtn.find('i');
    const isActive = filterBtn.data('active') === 'true';
    const cards = content.find('.storymode-blueprint-card');
    const emptyState = content.find('#library_empty_state');

    // Ensure button visual state matches data state (critical after refreshes)
    if (isActive && icon.length > 0) {
        filterBtn.addClass('storymode-filter-active');
        icon[0].style.setProperty('color', '#ffd700', 'important');
    } else if (icon.length > 0) {
        filterBtn.removeClass('storymode-filter-active');
    ...
    if (isActive) {
        ...
        cards.each(function () {
            const isFavorite = $(this).find('.storymode-card-favorite').hasClass('active');
            $(this).toggle(isFavorite);
        });

        // Update empty state if no favorites are visible
        const visibleCards = cards.filter(':visible');
        if (visibleCards.length === 0) {
            emptyState.find('i').removeClass('fa-folder-open').addClass('fa-star');
            emptyState.find('h3').text('No Favorites');
            emptyState.find('p').text('Mark scenarios as favorites by clicking the star icon on their cards.');
            emptyState.find('.storymode-generate-actions').hide();
            emptyState.show();
        }
    } else {
...
export function renderBlueprintGrid(content, blueprints) {
    ...
    if (blueprints.length === 0) {
        ...
        // Apply favorites filter if active (preserves filter state across refreshes)
...
export async function loadBlueprintFromLibrary(content, blueprintId, callbacks = {}) {
    ...
    try {
        ...
        // Use startStoryFromBlueprint for consistent resource detection and import prompts
        const result = await BlueprintModule.startStoryFromBlueprint(blueprint, { sourceType: 'library' });

        if (!result.success) {
            toastr.error(result.error || 'Failed to load blueprint');
            return;
        }

        // Show any warnings from resource validation
        if (result.warnings?.length > 0) {
            result.warnings.forEach(w => toastr.warning(w));
        }
 ...
 * Uses startStoryFromBlueprint for consistent resource detection and import prompts.
 *
 * @param {jQuery} content - The settings dialog content element
...
export async function playBlueprintFromLibrary(content, blueprintId, callbacks = {}) {
    ...
    try {
        ...
        // Check if blueprint will overwrite current settings
        const syncResult = await BlueprintModule.syncBlueprintSettings(blueprint, true);

        // Track whether we created a new chat (settings need to be applied to new context)
        ...
        // Use startStoryFromBlueprint for consistent resource detection and import prompts
        // This handles: missing resource detection, import prompts, scenario mode init, run copy
        // PERFORMANCE: Pass settingsAlreadySynced to avoid duplicate sync when settings were
        // already applied by the dialog above. Only false if we created a new chat.
        const settingsAlreadySynced = syncResult.confirmed && !createdNewChat;
        const result = await BlueprintModule.startStoryFromBlueprint(blueprint, {
            sourceType: 'library',
            settingsAlreadySynced
        });

        if (!result.success) {
            toastr.error(result.error || 'Failed to start story from blueprint');
            return;
        }
...
export async function deleteBlueprintFromLibrary(content, blueprintId, callbacks = {}) {
    ...
    const result = await callGenericPopup(
        'Delete this scenario blueprint from your library? This cannot be undone.',
        POPUP_TYPE.CONFIRM,
        '',
