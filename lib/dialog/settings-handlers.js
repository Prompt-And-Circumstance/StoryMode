...
export function setupUnifiedDialogEventListeners(content, context) {
    ...
    // ============================================================================
    // GENERIC API SETTING HANDLER (data-setting-path pattern)
    // ============================================================================

    // Generic handler for API dropdowns with data-setting-path attribute
    // This avoids adding individual handlers for each new API dropdown
    content.on('change', '.storymode-api-select[data-setting-path]', function() {
        const $select = $(this);
        const settingPath = $select.data('setting-path');  // e.g., "utilityApis.styleGeneration"
    ...
    // Helper: Rebuild Story Arc tab content (portable - accepts content param)
    ...
    // Helper: Rebuild Overview tab content after mode change
    ...
    // Helper: Switch pacing mode with all necessary updates (#1 - extracted)
    async function switchPacingMode(mode, activateBlueprint) {
    ...
    // Helper: Create feature toggle handler (#2 - extracted)
    ...
    // Go to Library button (from Overview tab when no blueprint loaded)
    content.on('click', '#go_to_library_btn', function() {
        libraryCallbacks.switchToTab(content, 'library');
    });

    // Quick action buttons (navigate to specific tabs/subtabs)
    content.on('click', '.storymode-quick-action', function() {
        ...
        if (gotoTab) {
            ...
            // If subtab specified, also switch to that subtab after a brief delay
            if (gotoSubtab) {
                setTimeout(() => {
    ...
    // Add missing style button (in Current Scenario panel)
    content.on('click', '.storymode-add-style-btn', async function(e) {
        e.stopPropagation();
        const btn = $(this);
        const styleType = btn.data('style-type'); // 'storyType' or 'authorStyle'

        // Get blueprint from current state
    ...
    // Feature toggles using extracted handler
    ...
    // Save group author style button (use event delegation)
    ...
    // Reset arc button (sidebar) - use event delegation for dynamically rendered content
    content.on('click', '#sidebar_reset_arc_btn', async function () {
    ...
    content.find('#blueprint_enabled, #blueprint_enabled_tab').on('change', function () {
        ...
        // Note: Do NOT call setupUnifiedDialogEventListeners here - it causes recursive handler registration
    });
    ...
    // Import blueprint button - supports both JSON and PNG (use event delegation)
    content.on('click', '#import_blueprint_btn', function () {
    ...
    content.on('click', '#blueprint_export_btn', async function () {
        ...
        try {
            // Export as PNG with embedded metadata (replaces old JSON export)
