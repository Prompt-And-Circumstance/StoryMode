...
import { saveSettingsDebounced, saveChatConditional, eventSource, characters, this_chid } from '/script.js';
...
import {
    ...
    setPacingMode,        // ADD THIS
    getPacingMode,        // ADD THIS
...
import {
    ...
    showLibraryGridView,
    renderMainPanel,
    buildStoryArcSubtab,  // ADD THIS - for rebuilding settings tab
} from '../ui/components.js';
import {
    updatePhaseStatusIndicators,
    resetPhaseOverrides,
} from '../ui/components/phase-override-panel.js';
import {
    debounce,
    refreshLibraryView,
...
export function setupUnifiedDialogEventListeners(content, context) {
    ...
    // Initialize character picker interactions (delegated, called once)
    ...
    console.log('[Story Mode] Setting up pacing mode handlers...');

    // Debug: Check if pacing mode buttons exist in content
    const storyBtn = content.find('#storymode-pacing-story');
    const scenarioBtn = content.find('#storymode-pacing-scenario');
    console.log('[Story Mode] Pacing mode buttons found:', {
        storyBtn: storyBtn.length,
        scenarioBtn: scenarioBtn.length
    });

    // Helper function to rebuild Story Arc tab content
    function rebuildStoryArcTab() {
        // The subtab container ID is generated by buildSubtabbedContent as settings_subtab_${id}
        const container = content.find('#settings_subtab_story_arc');
        if (container.length) {
            const newContent = buildStoryArcSubtab();
    ...
    // Pacing Mode: Story Mode button
    content.on('click', '#storymode-pacing-story', async function() {
        const pacingMode = getPacingMode();
        console.log('[Story Mode] Story button clicked, current pacingMode:', pacingMode);
        if (pacingMode === 'story') {
            console.log('[Story Mode] Already in Story Mode, no action needed');
            return;
        }

        // Switching from Scenario to Story
        ...
        try {
            ...
            // Rebuild settings UI and update other panels
            rebuildStoryArcTab();
            if (toastr) toastr.success('Switched to Story Mode');
            if (window.updateStatusDisplay) window.updateStatusDisplay();
            updateControllerPanel();
        } catch (error) {
            console.error('[Story Mode] Failed to switch to Story Mode:', error);
            if (toastr) toastr.error('Failed to switch mode: ' + error.message);
        }
    });

    // Pacing Mode: Scenario Mode button
    content.on('click', '#storymode-pacing-scenario', async function() {
        const pacingMode = getPacingMode();
        console.log('[Story Mode] Scenario button clicked, current pacingMode:', pacingMode);
        if (pacingMode === 'scenario') {
            console.log('[Story Mode] Already in Scenario Mode, no action needed');
            return;
        }

        // Check if blueprint is loaded
        const blueprintState = BlueprintModule.getBlueprintState();
        const hasBlueprint = blueprintState?.blueprint;

        if (!hasBlueprint) {
            // No blueprint - switch to library tab to select one
        ...
        try {
            // Blueprint exists - activate Scenario Mode
            blueprintState.useBlueprint = true;
            await BlueprintModule.saveBlueprintState(blueprintState);

            setPacingMode('scenario');
            saveSettingsDebounced();
            updateStoryPrompt();
            ...
            if (toastr) toastr.success('Switched to Scenario Mode');
            if (window.updateStatusDisplay) window.updateStatusDisplay();
            updateControllerPanel();
        } catch (error) {
            console.error('[Story Mode] Failed to switch to Scenario Mode:', error);
            if (toastr) toastr.error('Failed to switch mode: ' + error.message);
        }
    });

    // Stop Scenario button (when in Scenario Mode)
    content.on('click', '#stop_scenario_btn', async function() {
        const blueprintState = BlueprintModule.getBlueprintState();

        try {
            // Unload blueprint and switch to Story Mode
            blueprintState.useBlueprint = false;
            await BlueprintModule.saveBlueprintState(blueprintState);
            ...
            if (toastr) toastr.success('Scenario stopped, switched to Story Mode');
            if (window.updateStatusDisplay) window.updateStatusDisplay();
            updateControllerPanel();
        } catch (error) {
            console.error('[Story Mode] Failed to stop scenario:', error);
            if (toastr) toastr.error('Failed to stop scenario: ' + error.message);
        }
    });

    // View Blueprint button - switch to blueprint tab
    content.on('click', '#view_blueprint_btn', function() {
        libraryCallbacks.switchToTab(content, 'blueprint');
    });

    // Enable Story Type toggle
    content.on('change', '#story_type_enabled', function() {
        const enabled = $(this).prop('checked');
        extension_settings[MODULE_NAME].storyTypeEnabled = enabled;
        saveSettingsDebounced();

        // Show/hide story type controls
        if (enabled) {
            content.find('#story_type_controls').slideDown(200);
        } else {
            content.find('#story_type_controls').slideUp(200);
        }

        updateStoryPrompt();
        if (toastr) toastr.info(enabled ? 'Story Type enabled' : 'Story Type disabled');
        if (window.updateStatusDisplay) window.updateStatusDisplay();
        updateControllerPanel();
    });

    // Enable Story Arc toggle (existing - update to show/hide controls)
    content.find('#story_arc_enabled').on('change', function() {
        const enabled = $(this).is(':checked');
        extension_settings[MODULE_NAME].storyArcEnabled = enabled;
        saveSettingsDebounced();

        // Show/hide arc controls
        if (enabled) {
            content.find('#story_arc_controls').slideDown(200);
        } else {
            content.find('#story_arc_controls').slideUp(200);
        }

        updateStoryPrompt();
        if (toastr) toastr.info(enabled ? 'Story Arc enabled' : 'Story Arc disabled');
        updateStatusDisplay();
        updateControllerPanel();
    });
    ...
    // ========================================================================
    // Phase Override Panel Event Handlers
    // ========================================================================

    // Note: Expand/collapse is handled by SillyTavern's built-in inline-drawer system

    // Reset phase overrides to defaults
    ...
    // Update status indicators when profile or token values change
    ...
    // Return to Library button - from generate form
    ...
    // Add New Blueprint button - create blank blueprint and open editor
    content.on('click', '#library_add_new_btn, #library_empty_add_new_btn', async function (e) {
        ...
        try {
            ...
            const { normalizeBlueprint } = await import('../blueprint/normalization.js');
            ...
            // Open editor for the blank blueprint
            const edited = await openBlueprintEditor(blueprint);

            if (edited) {
                // Remove the _isNew flag before saving
                delete edited._isNew;
                // Save to library after editor closes with changes
