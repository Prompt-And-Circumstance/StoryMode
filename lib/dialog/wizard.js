...
export async function launchWizardModal(content, callbacks = {}) {
    ...
    if (retryBtn) {
        retryBtn.addEventListener('click', async function () {
            ...
            // Calculate token overrides for the failed phase (doubles tokens on retry)
            let tokenOverrides = {};
            const phaseConfig = PHASE_CONFIG[failedAtPhase];
            const defaultTokens = phaseConfig?.maxTokens || 8192;
            const currentTokens = phaseTokensUsed[failedAtPhase] || defaultTokens;
            const newTokens = Math.min(currentTokens * 2, MAX_PHASE_TOKENS);
            tokenOverrides[failedAtPhase] = newTokens;
            ...
            if (newTokens >= MAX_PHASE_TOKENS) {
                ...
                toastr.info(`Retrying ${phaseName} with ${newTokens} tokens (doubled from ${currentTokens})`);
            }
