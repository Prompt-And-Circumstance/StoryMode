    ...
    try {
        ...
        if (result.success) {
            ...
            if (validationErrors.length > 0) {
                ...
                console.log('[Story Mode] Validation warnings:', validationErrors);
                if (progressCont) progressCont.innerHTML = buildWizardProgressHTML(5);
                if (statusEl) statusEl.innerHTML = '<span style="color: #f59e0b;">⚠ Blueprint generated with warnings: ' + validationErrors.join(', ') + '</span>';
            } else {
                console.log('[Story Mode] Phase generation success. No validation errors.');
                if (progressCont) progressCont.innerHTML = buildWizardProgressHTML(5);
            }

            // Always offer cover generation if we have a cover_prompt (regardless of validation warnings)
            {
                console.log('[Story Mode] Checking cover generation...', { autoGenerate: extension_settings[MODULE_NAME]?.blueprintSettings?.coverGeneration?.autoGenerate, hasCoverPrompt: !!blueprint.cover_prompt });
