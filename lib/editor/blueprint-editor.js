...
// Wizard panel (for AI section generation)
import {
    renderWizardPanel,
    setupWizardPanelHandlers,
    setWizardRefreshFunctions,
    resetWizardPanelState,
} from './blueprint-editor/wizard-panel.js';
...
setWizardRefreshFunctions(refreshContent, refreshLeftPanel);
...
export async function openBlueprintEditor(blueprint) {
    setCurrentBlueprint(cloneBlueprint(blueprint));
    setOriginalBlueprint(cloneBlueprint(blueprint));
    ...
    // Reset wizard panel state (auto-detect if should be open based on blueprint)
    resetWizardPanelState();
    ...
    <div class="storymode-blueprint-editor-container">
        <div class="storymode-editor-title-bar">
            ...
            <div class="storymode-editor-title-actions">
                ...
                <button class="menu_button" id="blueprint_duplicate_btn" title="Create a copy in library">
                    <i class="fa-solid fa-copy"></i> Duplicate
                </button>
                <button class="menu_button" id="blueprint_export_btn" title="Export as PNG with embedded resources">
                    <i class="fa-solid fa-download"></i> Export
                </button>
                <button class="storymode-btn-start" id="blueprint_set_as_current_btn" title="Load this blueprint as the current scenario" style="margin-left: 16px;">
                    <i class="fa-solid fa-play"></i>&nbsp;Load Story
                </button>
            </div>
        </div>
 ...
 * Scoped to blueprint editor tabs only (not settings dialog tabs)
 */
function updateUnsavedIndicator() {
    const indicator = $('.storymode-editor-tabs .storymode-unsaved-indicator');
    if (getHasUnsavedChanges() && indicator.length === 0) {
        // Add indicator to active tab within the editor only
        $('.storymode-editor-tabs .storymode-tab.active').append('<span class="storymode-unsaved-indicator"></span>');
