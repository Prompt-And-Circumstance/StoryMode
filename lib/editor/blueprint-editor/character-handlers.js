...
export function setupCharacterTabHandlers(EVENT_NAMESPACE) {
    ...
    // Delink character (click on linked card — NOT in delinked section or missing section)
    $(document).on('click' + EVENT_NAMESPACE, '.characters-section:not(.delinked-section) .character-mini-card[data-delink-name]', async function (e) {
        e.preventDefault();
        if (!$(this).closest('.storymode-characters-tab').length) return; // Scope to editor
        const name = $(this).data('delink-name');
        if (!name) return;
        toggleDelink(name);
        toastr.info(`${name} delinked. Character data will not be included in exports. Arcs and scenes are preserved.`);
        await loadCharactersTabContent();
    });

    // Relink character (click on delinked card)
    $(document).on('click' + EVENT_NAMESPACE, '.delinked-section .character-mini-card[data-delink-name]', async function (e) {
        e.preventDefault();
        if (!$(this).closest('.storymode-characters-tab').length) return;
        const name = $(this).data('delink-name');
        if (!name) return;
        toggleDelink(name);
        toastr.success(`${name} relinked to blueprint.`);
        await loadCharactersTabContent();
    });
}
