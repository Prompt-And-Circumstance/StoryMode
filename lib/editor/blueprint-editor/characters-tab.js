...
function renderCharactersSubTab(bp, linkInfo) {
    return `
        ...
        <div class="storymode-form-section">
            ...
            ${renderLinkedCharacters(linkInfo)}
        </div>
    `;
}
 ...
 */
function renderLinkedCharacters(linkInfo) {
    ...
    return `
        ${hasLinked ? `
            <div class="characters-section">
                <h5><i class="fa-solid fa-check-circle" style="color: var(--SmartThemeEmColor);"></i> Characters (${linkInfo.linked.length})</h5>
        ...
        ${hasPersonas ? `
            <div class="characters-section">
                <h5><i class="fa-solid fa-masks-theater" style="color: var(--SmartThemeEmColor);"></i> Personas (${linkInfo.linkedPersonas.length})</h5>
                <div class="characters-grid">
                    ${linkInfo.linkedPersonas.map(link => renderPersonaCard(link.localPersona)).join('')}
                </div>
            </div>
        ` : ''}
        ...
        ${hasMissing ? `
            <div class="characters-section">
                ...
                <p class="missing-hint">
                    ...
                    These characters are referenced in arcs but not found in your library.
                </p>
            </div>
        ` : ''}
    `;
...
export function renderPersonaCard(persona) {
    ...
    const displayName = persona.title ? `${persona.name} (${persona.title})` : persona.name;
    return `
        <div class="character-mini-card available persona-card">
