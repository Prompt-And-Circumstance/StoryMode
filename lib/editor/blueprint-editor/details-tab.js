...
export function renderDetailsTab() {
    ...
    // Build dropdowns using helper (sorted alphabetically)
    const storyTypeOptions = [...storyTypes].sort((a, b) => a.name.localeCompare(b.name)).map(st =>
        `<option value="${st.id}" ${bp.story_type_id === st.id ? 'selected' : ''}>${escapeHtml(st.name)}</option>`
    ).join('');

    const authorStyleOptions = [...authorStyles].sort((a, b) => a.name.localeCompare(b.name)).map(as =>
