...
/**
 * Build scene count dropdown options
 * @param {number} currentValue - Current scene count
 * @returns {string} HTML options
 */
function buildSceneCountOptions(currentValue) {
    const presets = [
        { value: 5, label: 'Short (5 scenes)' },
        { value: 10, label: 'Medium (10 scenes)' },
        { value: 15, label: 'Long (15 scenes)' },
        { value: 20, label: 'Very Long (20 scenes)' }
    ];

    // Find closest preset or default to medium
    ...
    return presets.map(p =>
        `<option value="${p.value}" ${p.value === closestPreset.value ? 'selected' : ''}>${p.label}</option>`
    ).join('');
}

// styleExistsInLibrary removed - now uses isStyleMissing from missing-style-handler.js

/**
 * Build style dropdown options with support for missing styles (embedded or not)
 * @param {Array} libraryStyles - Styles from user's library
 * @param {string} selectedId - Currently selected style ID
 * @param {Object} embeddedStyle - Embedded style from blueprint (may be null)
 * @param {string} styleName - Name hint from blueprint (for non-embedded styles)
 * @param {string} type - 'storyType' or 'authorStyle'
 * @returns {{options: string, hasMissing: boolean, missingStyle: Object|null, hasEmbedded: boolean}}
 */
function buildStyleDropdownOptions(libraryStyles, selectedId, embeddedStyle, styleName = '', type = '') {
    const sorted = [...libraryStyles].sort((a, b) => a.name.localeCompare(b.name));
    let hasMissing = false;
    let missingStyle = null;
    let hasEmbedded = false;

    // Check if selected style is missing from library (use centralized handler)
    if (selectedId && isStyleMissing(type, selectedId)) {
        hasMissing = true;

        // Check if we have embedded data for this style
        if (embeddedStyle?.id === selectedId) {
            hasEmbedded = true;
            missingStyle = embeddedStyle;
        } else {
            // No embedded data - create a placeholder object for display
            hasEmbedded = false;
            missingStyle = {
                id: selectedId,
                name: styleName || selectedId
            };
        }
    }

    let options = sorted.map(s =>
        `<option value="${s.id}" ${selectedId === s.id ? 'selected' : ''}>${escapeHtml(s.name)}</option>`
    ).join('');

    // Add missing style as a special option at the top
    if (hasMissing && missingStyle) {
        const notEmbeddedNote = hasEmbedded ? '' : ' - Not embedded';
        const missingOption = `<option value="${missingStyle.id}" selected style="color: var(--SmartThemeWarningColor);">${escapeHtml(missingStyle.name)} (Not in library${notEmbeddedNote})</option>`;
        options = missingOption + options;
    }

    return { options, hasMissing, missingStyle, hasEmbedded };
}
...
export function renderDetailsTab() {
    ...
    // Metadata for Creator Notes section
    ...
    const generatedBy = bp.llmDescriptor || 'Unknown';

    // Build dropdowns with missing style detection
    const embeddedStoryType = bp.embeddedResources?.storyType || null;
    const embeddedAuthorStyle = bp.embeddedResources?.authorStyle || null;

    const storyTypeDropdown = buildStyleDropdownOptions(
        storyTypes, bp.story_type_id, embeddedStoryType, bp.story_type_name || '', 'storyType'
    );
    const authorStyleDropdown = buildStyleDropdownOptions(
        authorStyles, bp.author_style, embeddedAuthorStyle, bp.author_style_name || '', 'authorStyle'
    );

    // Legacy variable names for compatibility
    const storyTypeOptions = storyTypeDropdown.options;
    const authorStyleOptions = authorStyleDropdown.options;
    ...
    return `
        ...
        <div class="storymode-form-section">
            ...
            <!-- Metadata (read-only) -->
            <div class="storymode-metadata-row">
                <span class="storymode-metadata-item"><i class="fa-solid fa-calendar-plus"></i> Created: ${escapeHtml(createdDate)}</span>
                <span class="storymode-metadata-item"><i class="fa-solid fa-calendar-check"></i> Modified: ${escapeHtml(modifiedDate)}</span>
                <span class="storymode-metadata-item"><i class="fa-solid fa-robot"></i> Generated by: ${escapeHtml(generatedBy)}</span>
            </div>
        </div>
            ...
            <div class="storymode-form-grid">
                <div>
                    ...
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <select id="edit_story_type" class="text_pole" data-field="story_type_id" style="flex: 1;">
                            ${storyTypeOptions}
                        </select>
                        ${storyTypeDropdown.hasMissing ? `
                            <button id="add_story_type_btn" class="menu_button"
                                title="Add '${escapeHtml(storyTypeDropdown.missingStyle?.name || '')}' to your Story Types library"
                                data-style-id="${escapeHtml(storyTypeDropdown.missingStyle?.id || '')}"
                    ...
                    ${storyTypeDropdown.hasMissing ? `
                        <p class="storymode-form-hint" style="margin-top: 4px; color: var(--SmartThemeWarningColor);">
                            <i class="fa-solid fa-triangle-exclamation"></i> This story type is not in your library.
                            ${storyTypeDropdown.hasEmbedded
                                ? 'Click "Add" to import it.'
                                : 'Click "Add" to create it manually (no embedded data available).'}
                        </p>
                    ` : ''}
                </div>
                <div>
                    ...
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <select id="edit_author_style" class="text_pole" data-field="author_style" style="flex: 1;">
                            <option value="" ${!bp.author_style ? 'selected' : ''}>None</option>
                            ${authorStyleOptions}
                        </select>
                        ${authorStyleDropdown.hasMissing ? `
                            <button id="add_author_style_btn" class="menu_button"
                                title="Add '${escapeHtml(authorStyleDropdown.missingStyle?.name || '')}' to your Author Styles library"
                                data-style-id="${escapeHtml(authorStyleDropdown.missingStyle?.id || '')}"
                                style="padding: 4px 8px; white-space: nowrap;">
                                <i class="fa-solid fa-plus"></i> Add
                            </button>
                        ` : ''}
                    ...
                    ${authorStyleDropdown.hasMissing ? `
                        <p class="storymode-form-hint" style="margin-top: 4px; color: var(--SmartThemeWarningColor);">
                            <i class="fa-solid fa-triangle-exclamation"></i> This author style is not in your library.
                            ${authorStyleDropdown.hasEmbedded
                                ? 'Click "Add" to import it.'
                                : 'Click "Add" to create it manually (no embedded data available).'}
                        </p>
                    ` : ''}
        ...
        <div class="storymode-form-section">
            ...
            <div class="storymode-form-grid">
                <div>
                    <label for="edit_scene_count">Story Length (scenes)</label>
                    <select id="edit_scene_count" class="text_pole" style="width: 100%;">
                        ${buildSceneCountOptions(bp.scene_plan?.length || getNestedValue(bp, 'arc_structure.total_messages_target', 10))}
                    </select>
                </div>
                <div>
                    <label for="edit_custom_scenes">Custom Scenes</label>
                    <input type="number" id="edit_custom_scenes" class="text_pole" placeholder="Override scene count"
                        min="1" max="100" style="width: 100%;">
                    <p class="storymode-form-hint" style="margin-top: 4px; font-size: 0.8em;">Leave empty to use dropdown value</p>
                </div>
            </div>
            <div class="storymode-form-grid" style="margin-top: 12px;">
                <div>
                ...
                <div>
                    ...
                    <div class="text_pole" style="background: var(--SmartThemeBlurTintColor); padding: 8px; text-align: center;">
                        ${bp.scene_plan?.length || 0} scenes defined
                    </div>
                </div>
            </div>
        </div>
