...
import { escapeHtml, cloneBlueprint, sanitizeFilename, downloadBlob, isDelinked } from '../../blueprint/utils.js';
import {
    getCurrentBlueprint,
    setCurrentBlueprint,
...
import { resolveStoryType, resolveAuthorStyle, buildAuthorStyleFromInlineFields } from '../../blueprint/resource-utils.js';
...
function gatherJsonMetadata(blueprint) {
    ...
    if (blueprint.selectedPersonas?.length > 0 || blueprint.personaData?.length > 0) {
        ...
        if (personaNames.length > 0) {
            enriched.embeddedResources.personas = extractPersonaMetadata(personaNames);
        }
    }

    // Story type - three-tier fallback (library → inline → previous embedded)
    const resolvedStoryType = resolveStoryType(blueprint, getStoryTypes());
    if (resolvedStoryType) enriched.embeddedResources.storyType = resolvedStoryType;

    // Author style - three-tier fallback, plus no-ID inline path
    const resolvedAuthorStyle = resolveAuthorStyle(blueprint, getAuthorStyles());
    if (resolvedAuthorStyle) {
        enriched.embeddedResources.authorStyle = resolvedAuthorStyle;
    } else if (blueprint.author_style_name && blueprint.author_style_prompt) {
        enriched.embeddedResources.authorStyle = buildAuthorStyleFromInlineFields(blueprint, `inline_${Date.now()}`);
    }
