...
async function handleDuplicateBlueprint(e) {
    ...
    const confirmed = await callGenericPopup(
        ...
        <p>Create a copy of "<strong>${escapeHtml(blueprint.userMetadata?.title || 'Untitled')}</strong>" in your library?</p>
    ...
    try {
        // Import the duplicateBlueprint function
        ...
        toastr.success(`Created duplicate: ${newBlueprint.userMetadata?.title || 'Blueprint'}`);

        // Optionally ask if user wants to edit the new copy
        const editNew = await callGenericPopup(
            `<h3>Blueprint Duplicated</h3>
            <p>The copy "<strong>${escapeHtml(newBlueprint.userMetadata?.title || 'Untitled')}</strong>" has been saved to your library.</p>
        ...
        if (editNew) {
            // Update the editor to show the new blueprint
            try {
                setCurrentBlueprint(structuredClone(newBlueprint));
            } catch (err) {
                setCurrentBlueprint(JSON.parse(JSON.stringify(newBlueprint)));
            }
            setHasUnsavedChanges(false);
            if (_refreshEditor) _refreshEditor();
        }
