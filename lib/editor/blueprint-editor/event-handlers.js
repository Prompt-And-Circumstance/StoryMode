...
import { renderCharactersTab, setupCharacterTabHandlers as setupCharTabHandlers } from './characters-tab.js';
...
function setupBeatDragDrop() {
    let dragState = { card: null, index: null };

    const clearState = () => {
        $('.storymode-beat-card').removeClass('dragging drag-over');
        dragState = { card: null, index: null };
    };

    // Attach dragstart to the handle, but track the parent card
    $(document).on('dragstart' + EVENT_NAMESPACE, '.storymode-beat-drag-handle', function (e) {
        ...
        e.originalEvent.dataTransfer.setData('text/plain', dragState.index);
    });

    $(document).on('dragend' + EVENT_NAMESPACE, '.storymode-beat-drag-handle', clearState);

    // Drop targets are the cards themselves
    $(document).on('dragover' + EVENT_NAMESPACE, '.storymode-beat-card', function (e) {
        e.preventDefault();
        e.originalEvent.dataTransfer.dropEffect = 'move';
        if (dragState.card && !dragState.card.is($(this))) {
    ...
    $(document).on('drop' + EVENT_NAMESPACE, '.storymode-beat-card', function (e) {
        ...
        if (dragState.card && !dragState.card.is($(this))) {
            ...
            if (targetIndex !== dragState.index && dragState.index !== null) {
...
function setupSceneDragDrop() {
    ...
    // Attach dragstart to the handle, but track the parent card
    $(document).on('dragstart' + EVENT_NAMESPACE, '.storymode-scene-drag-handle', function (e) {
    ...
    $(document).on('dragend' + EVENT_NAMESPACE, '.storymode-scene-drag-handle', clearState);
