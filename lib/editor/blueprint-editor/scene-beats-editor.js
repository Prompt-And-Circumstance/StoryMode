...
export function renderBeatsEditor(beats = []) {
    if (!beats || beats.length === 0) {
        return `
            <div class="storymode-beats-section" style="margin-top: 16px; padding: 12px; border-radius: 4px; border-left: 3px solid var(--SmartThemeEmColor);">
    ...
    return `
        <div class="storymode-beats-section" style="margin-top: 16px;">
            ...
            <div class="storymode-beats-list" style="display: flex; flex-direction: column; gap: 8px;">
                ${beats.map((beat, idx) => `
                    <div class="storymode-beat-card" data-beat-index="${idx}" draggable style="padding: 8px; border-left: 3px solid var(--SmartThemeEmColor); border-radius: 2px; display: flex; justify-content: space-between; align-items: flex-start;">
                        <div style="flex: 1; cursor: grab;">
                            <i class="fa-solid fa-grip-vertical" style="margin-right: 8px; color: var(--SmartThemeEmColor); opacity: 0.6;"></i>
                            <strong>${escapeHtml(beat.title || 'Untitled Beat')}</strong>
                            ${beat.type ? `<span style="margin-left: 8px; padding: 2px 6px; background-color: var(--SmartThemeEmColor); color: #fff; border-radius: 3px; font-size: 0.8em;">${escapeHtml(beat.type)}</span>` : ''}
                            ${beat.required ? `<span style="margin-left: 8px; padding: 2px 6px; background-color: #d9534f; color: #fff; border-radius: 3px; font-size: 0.8em;">Required</span>` : ''}
                            ${beat.description ? `<div style="margin-top: 4px; font-size: 0.9em; color: var(--SmartThemeEmColor);">${escapeHtml(beat.description)}</div>` : ''}
                        </div>
                        <div style="display: flex; gap: 4px; margin-left: 8px;">
                            <button type="button" class="menu_button" data-beat-action="edit" data-beat-index="${idx}" title="Edit beat" style="padding: 4px 6px;">
                                <i class="fa-solid fa-pen"></i>
                            </button>
                            <button type="button" class="menu_button" data-beat-action="delete" data-beat-index="${idx}" title="Delete beat" style="padding: 4px 6px;">
