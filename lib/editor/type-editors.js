...
<div style="text-align: center; padding: 10px;">
    ...
    <small class="notes" style="display: block; margin-top: 10px;">
        ...
        <strong>Merge:</strong> Keep existing ${itemType}, update duplicates, add new ones.
    </small>
</div>
`;
    const content = $(html);
 ...
 * Prompts user to replace all or merge with existing types.
 *
 * @async
...
async function importStoryTypes(fileInput) {
    ...
    try {
        ...
        // Validate all types before prompting
        ...
        const choice = await showImportChoicePopup('story types', imported.length);
        if (choice === null) {
            $(fileInput).val('');
        ...
        if (choice === 'replace') {
            ...
            // Merge: existing types updated, new types appended
            finalTypes = [...getStoryTypes()];
            for (const type of imported) {
                const existing = finalTypes.findIndex(t => t.id === type.id);
                if (existing >= 0) {
                    finalTypes[existing] = type;
                } else {
        ...
        const action = choice === 'replace' ? 'Replaced with' : 'Merged';
        toastr.success(`${action} ${imported.length} story types`);
 ...
 * Prompts user to replace all or merge with existing styles.
 *
 * @async
 * @param {HTMLInputElement} fileInput - The file input element containing the JSON file.
 * @returns {Promise<void>}
...
async function importAuthorStyles(fileInput) {
    ...
    try {
        ...
        // Validate all styles before prompting
        ...
        // Ask user whether to replace or merge
        const choice = await showImportChoicePopup('author styles', imported.length);
        if (choice === null) {
            $(fileInput).val('');
            return; // User cancelled
        }
        ...
        let finalStyles;
        if (choice === 'replace') {
            finalStyles = imported;
        } else {
            // Merge: existing styles updated, new styles appended
            finalStyles = [...getAuthorStyles()];
            for (const style of imported) {
                const existing = finalStyles.findIndex(s => s.id === style.id);
                if (existing >= 0) {
                    finalStyles[existing] = style;
                } else {
        ...
        const action = choice === 'replace' ? 'Replaced with' : 'Merged';
        toastr.success(`${action} ${imported.length} author styles`);
