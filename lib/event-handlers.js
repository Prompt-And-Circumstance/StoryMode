    ...
    for (const match of beatMatches) {
        ...
        if (scene.beats && beatIndex >= 0 && beatIndex < scene.beats.length) {
            ...
            if (newlyCompleted) {
                ...
                const logMessage = `Beat completed: Scene ${currentSceneIndex + 1}, Beat ${beatIndex + 1}: "${beat.title}"`;
                console.log(`[Story Mode] ${logMessage}`);

                // Debug notifications if enabled
                if (settings.debugMode) {
                    toastr.success(
                        `Beat ${beatIndex + 1}: ${beat.title}`,
                        `Scene ${currentSceneIndex + 1} - Beat Completed`,
                        { timeOut: 4000, extendedTimeOut: 2000 }
                    );
                    console.log(`[Story Mode Debug] 🎯 BEAT COMPLETED`, {
                        scene: currentSceneIndex + 1,
                        sceneTitle: scene.title,
                        beat: beatIndex + 1,
                        beatTitle: beat.title,
                        beatType: beat.type || 'standard',
        ...
        if (blueprintState?.blueprint && blueprintState.useBlueprint) {
            ...
            if (message?.mes && !message.is_user) {
                ...
                if (message.mes.includes(SCENE_MARKER)) {
                    ...
                    // Debug logging for scene transitions
                    if (settings.debugMode) {
                        console.log(`[Story Mode Debug] 🎬 SCENE TRANSITION`, {
                            from: {
                                index: currentSceneIndex + 1,
                                title: blueprint.scene_plan[currentSceneIndex]?.title || 'Unknown'
                            },
                            to: {
                                index: newSceneIndex + 1,
                                title: newScene.title,
                                phase: newScene.phase,
                                purpose: newScene.purpose,
                                beats: newScene.beats?.length || 0
                            },
                            mode: blueprintState.sceneMode,
                            wasAutoMode
                        });
                    }
                    ...
                    if (notifyType === 'none') {
                        // No notification by default, but show in debug mode
                        if (settings.debugMode) {
                            toastr.info(
                                `Now: ${newScene.title} (${newScene.phase})`,
                                `Scene ${newScene.index + 1}/${blueprint.scene_plan.length}`,
