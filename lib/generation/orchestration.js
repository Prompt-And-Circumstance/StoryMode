...
// ============================================================
// LORE CONTEXT BUILDING
// ============================================================

// Maximum characters per lore entry and total lore context to prevent prompt bloat
const MAX_LORE_ENTRY_LENGTH = 2000;
const MAX_TOTAL_LORE_LENGTH = 10000;

/**
 * Build lore context string from lorebook data
 * @param {Object|null} embeddedLorebook
 * @param {Array<string>} linkedLorebooks
 * @returns {string} Formatted lore context
 */
function buildLoreContext(embeddedLorebook, linkedLorebooks) {
    const loreSections = [];

    // Embedded lorebook entries
    if (embeddedLorebook?.entries?.length > 0) {
        const entries = embeddedLorebook.entries
            .filter(e => !e.disable)
            .sort((a, b) => (a.order || 100) - (b.order || 100));

        const entryTexts = entries.map(e => {
            const keywords = e.key.join(', ');
            // Truncate content to prevent excessive prompt length
            const content = e.content && e.content.length > MAX_LORE_ENTRY_LENGTH
                ? e.content.substring(0, MAX_LORE_ENTRY_LENGTH) + '...[truncated]'
                : e.content || '';
            return `[${e.comment}] (${keywords})\n${content}`;
        });

        const fullContext = `# World Lore Context\n\n` +
            `The following worldinfo entries provide context about the established world:\n\n` +
            entryTexts.join('\n\n---\n\n');

        // Truncate total lore context if needed
        loreSections.push(
            fullContext.length > MAX_TOTAL_LORE_LENGTH
                ? fullContext.substring(0, MAX_TOTAL_LORE_LENGTH) + '\n\n...[truncated]'
                : fullContext
        );
    }

    // Linked lorebooks (future: fetch and format)
    if (linkedLorebooks.length > 0) {
        // For now, just mention them
        loreSections.push(
            `\n\n# Linked Lorebooks\n` +
            `This blueprint references the following lorebooks: ${linkedLorebooks.join(', ')}\n` +
            `(These will be activated during story play via SillyTavern's World Info system)`
        );
    }

    return loreSections.join('\n\n');
}
...
export async function generateWithPreset(options) {
    ...
    // Fall back to Main API when no Connection Manager profile is configured
    if (!selectedProfileId) {
        console.warn(`[Story Mode Blueprint] No profile configured — falling back to Main API for Phase ${options.phase || '?'} (${options.phaseName || 'Unknown'})`);
...
function addBlueprintMetadata(blueprint, request, storyTypes, authorStyles, selectedProfileId) {
    ...
    // Attach lorebook data to final blueprint
    if (request.embedded_lorebook) {
        blueprint.embedded_lorebook = request.embedded_lorebook;
    }
    if (request.linked_lorebooks && request.linked_lorebooks.length > 0) {
        blueprint.linked_lorebooks = request.linked_lorebooks;
    }
}
...
export async function generateBlueprintPhased(request, storyTypes, authorStyles, onPhaseUpdate, options = {}) {
    ...
    try {
        ...
        // Build lore context for prompts
        const loreContext = buildLoreContext(embedded_lorebook, linked_lorebooks);

        // Add lore context to request for prompt building
        const requestWithLore = loreContext
            ? { ...request, loreContext }
            : request;
            ...
            try {
                ...
                if (phaseNum === 3 && (shouldUseStagedGeneration(request) || stagedRetry)) {
                    ...
                    const stagedResult = await executeStagedPhase3(
                        requestWithLore, storyType, authorStyle, partialBlueprint,
                    ...
                    const phaseResult = await executePhase(phaseNum, requestWithLore, storyType, authorStyle, partialBlueprint, selectedProfileId, phaseOverrides);
