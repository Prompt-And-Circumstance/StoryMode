...
export function getExpectedSceneCount(sceneTarget) {
    ...
    // Feasible ranges: 5, 10, 15, 20 scenes
    if (sceneTarget <= 5) {
        return 'exactly 5 scenes';
    } else if (sceneTarget <= 10) {
        return 'exactly 10 scenes';
    } else if (sceneTarget <= 15) {
        return 'exactly 15 scenes';
    } else if (sceneTarget <= 20) {
        return 'exactly 20 scenes';
    } else {
        // Fallback for legacy or custom values
        return `approximately ${sceneTarget} scenes`;
    }
}
