...
const VALID_TEMPLATE_PATHS = new Set([
    ...
    'phased-generation/scene-plan-prompt.txt',
    'phased-generation/scene-batch-prompt.txt',
    'phased-generation/resolutions-prompt.txt',
    'style-generation/author-style-prompt.txt',
    'style-generation/story-type-prompt.txt',
]);
...
export async function getSummaryRequirements(style = SummaryStyle.NARRATIVE) {
    ...
    const customKey = `summaryRequirements_${style}`;
    if (settings.blueprintSettings?.[customKey]) {
        return settings.blueprintSettings[customKey];
    }
