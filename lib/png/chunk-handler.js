...
/**
 * Replace all metadata chunks in a PNG with new ones
 *
 * Unlike insertMetadataChunks, this removes existing tEXt, iTXt, and zTXt chunks
 * before inserting new metadata. Use this when updating metadata to avoid
 * duplicate chunks where the old values would overwrite the new ones during decode.
 *
 * @param {Uint8Array} originalPngBytes - Original PNG file bytes
 * @param {Uint8Array[]} metadataChunks - New metadata chunks to insert
 * @returns {Uint8Array} New PNG bytes with metadata replaced
 */
export function replaceMetadataChunks(originalPngBytes, metadataChunks) {
    verifyPNGSignature(originalPngBytes);

    const METADATA_TYPES = new Set([ChunkType.TEXT, ChunkType.INTERNATIONAL_TEXT, 'zTXt']);
    const preservedChunks = [];
    let ihdrChunk = null;
    let offset = 8; // Skip signature

    // Parse all chunks, keeping non-metadata chunks
    while (offset < originalPngBytes.length) {
        const length = readUInt32BE(originalPngBytes, offset);
        const type = new TextDecoder().decode(originalPngBytes.slice(offset + 4, offset + 8));
        const totalLength = 12 + length; // length(4) + type(4) + data(N) + crc(4)
        const chunkBytes = originalPngBytes.slice(offset, offset + totalLength);

        if (type === ChunkType.IMAGE_HEADER) {
            ihdrChunk = chunkBytes;
        } else if (!METADATA_TYPES.has(type)) {
            // Keep IDAT, pHYs, IEND, etc. — drop metadata chunks
            preservedChunks.push(chunkBytes);
        }

        offset += totalLength;
        if (type === ChunkType.END) break;
    }

    if (!ihdrChunk) {
        throw new Error('[PNGChunkHandler] No IHDR chunk found in PNG');
    }

    // Calculate total size
    const signatureSize = 8;
    const metadataSize = metadataChunks.reduce((acc, c) => acc + c.length, 0);
    const preservedSize = preservedChunks.reduce((acc, c) => acc + c.length, 0);
    const newBytes = new Uint8Array(signatureSize + ihdrChunk.length + metadataSize + preservedSize);

    // Assemble: Signature → IHDR → New Metadata → Other Chunks (including IEND)
    let pos = 0;

    // Signature
    newBytes.set(originalPngBytes.slice(0, 8), pos);
    pos += 8;

    // IHDR (must be first chunk)
    newBytes.set(ihdrChunk, pos);
    ...
    // Preserved chunks (IDAT, pHYs, IEND, etc.)
 ...
 * - replaceMetadataChunks(originalPngBytes, metadataChunks) -> Uint8Array (strips old metadata first)
 *
 * Constants:
