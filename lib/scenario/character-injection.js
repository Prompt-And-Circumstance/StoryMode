...
import { isDelinked } from '../blueprint/utils.js';
...
export function getMissingCharactersForInjection(blueprint) {
    ...
    // Filter out delinked characters from both linked and missing
    const filteredLinked = linkResult.linked.filter(
        ({ blueprintName }) => !isDelinked(blueprint, blueprintName)
    );
    const filteredMissing = linkResult.missing.filter(
        name => !isDelinked(blueprint, name)
    );
    ...
    for (const { blueprintName, localCharacter } of filteredLinked) {
    ...
    missingNames.push(...filteredMissing);
