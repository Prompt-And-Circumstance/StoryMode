...
export function showImagePreviewPopup(sceneIndex) {
    ...
    console.log(`[Scene Image Preview] Showing preview for scene ${sceneIndex}`);
}
...
function renderPreviewContent(scene, imageData, blueprint) {
    ...
    const characterOptions = buildCharacterOptions(scene, context);
    ...
    return `
        ...
        <div class="storymode-image-preview-actions">
            ...
            <button class="storymode-add-to-chat-btn" title="Add image to the current chat message">
                <i class="fa-solid fa-comment-image"></i> Add to Chat
            </button>
 ...
 */
function buildCharacterOptions(scene, context) {
    const options = [];
    const addedNames = new Set();

    // Try to get current chat characters from SillyTavern context
    // The context provides name2 (character name) and characterId
    if (context.name2) {
        // Single character chat - name2 is the character name
    ...
    // Try group members if available
    if (context.groups) {
    ...
    // Add characters from scene focus if blueprint has character data
    ...
    // Fallback: Add characters array entries if available
...
function bindPreviewEvents(popup, blueprint, scene, imageData) {
    ...
    // Download button
    popup.find('.storymode-download-btn').on('click', () => {
        downloadImage(imageData.imageData, `scene-${scene.index + 1}-${blueprint.blueprint_id.slice(0, 8)}`);
    });
    ...
    popup.find('.storymode-add-gallery-btn').on('click', function () {
        ...
        addToCharacterGallery(characterName, imageData.imageData, scene);
    });
 ...
 * @param {Object} scene - Scene object
 */
function addToCharacterGallery(characterName, imageData, scene) {
    // This would integrate with SillyTavern's character gallery system
    // For now, show a message indicating the feature
    if (window.toastr) {
        toastr.info(`Gallery integration for "${characterName}" coming soon!`);
    }
    console.log(`[Scene Image Preview] Would add image to ${characterName}'s gallery for scene ${scene.index}`);
}
...
async function addImageToChat(imageUrl, scene, blueprint) {
    ...
    if (targetMessageIndex === -1) {
        throw new Error('No AI message found to attach image to');
    }

    const message = context.chat[targetMessageIndex];
    ...
    console.log(`[Scene Image Preview] Added scene ${scene.index + 1} image to message ${targetMessageIndex}`);
}
