...
<div class="inline-drawer ${openClass}" id="${this.escapeHtml(id)}">
    ...
    /**
     * Flat card section (non-collapsible)
     * @param {Object} props - Component properties
     * @param {string} [props.id] - Optional card ID
     * @param {string} props.title - Card title
     * @param {string} [props.icon] - Font Awesome icon class
     * @param {string} props.content - Card HTML content
     * @param {boolean} [props.fullWidth=false] - Whether card spans full width in grid
     * @returns {string} HTML string
     */
    card(props) {
        const { id, title, icon, content, fullWidth = false } = props;
        const iconHtml = icon ? `<i class="${icon}"></i>` : '';
        const idAttr = id ? `id="${this.escapeHtml(id)}"` : '';
        const widthClass = fullWidth ? ' full-width' : '';

        return `
<div class="storymode-form-card${widthClass}" ${idAttr}>
    <div class="storymode-form-card-header">
        ${iconHtml}
        <h4>${this.escapeHtml(title)}</h4>
    </div>
    <div class="storymode-form-card-body">
        ${content}
    </div>
</div>`;
    },
...
// Export ComponentTemplates as 'ui' for convenient access
export const ui = ComponentTemplates;
