...
import { LOADING_INDICATOR_MODULE, defaultIndicatorSettings } from './loading-indicator.js';
...
export function createToggle({ id, label, description, helpText, checked = true }) {
    ...
    const helpIcon = helpText ? createHelpIcon(helpText) : '';
    return `
<div class="storymode-toggle">
<div class="storymode-toggle-info">
<span class="storymode-toggle-label">${escapeHtml(label)}</span>
<span class="storymode-toggle-description">${escapeHtml(description)}</span>
${helpIcon}
</div>
<label class="storymode-switch">
<input type="checkbox" id="${escapeHtml(id)}" ${checkedAttr}>
 ...
 * @param {boolean|undefined} value - The checkbox value (undefined treated as true for defaults)
 ...
 * Returns 'checked' unless the value is explicitly false
...
export function renderBlueprintPreview(blueprintState) {
    ...
    const showGenerateOpening = currentStep === 0 && currentScene && currentScene.index === 0;
    ...
    <div class="storymode-blueprint-preview inline-drawer">
        ...
        <div class="inline-drawer-content" style="display: none;">
            ...
            <div style="margin-top: 12px;">
                <button id="generate_opening_btn" class="menu_button" style="width: 100%;">
                    <i class="fa-solid fa-play"></i> Generate Opening Message…
                </button>
                <div id="generate_opening_status" style="margin-top: 8px; font-size: 0.85em; color: var(--SmartThemeQuoteColor); text-align: center; display: none;"></div>
            </div>
            ` : ''}
        </div>
    </div>
...
<div class="storymode-form-group">
<label class="storymode-form-label">Opening Message Profile ${createHelpIcon('Which API connection to use for generating the opening scene message')}</label>
...
<!-- Advanced Settings -->
<details class="storymode-advanced">
<summary>Advanced Settings</summary>
<div class="storymode-advanced-content">
<!-- Injection Settings -->
<div class="storymode-card">
...
'Assistant: As assistant guidance'
])}</label>
...
<label class="storymode-switch">
<input type="checkbox" id="debug_mode_enabled">
<span class="storymode-switch-slider"></span>
</label>
</div>
<!-- Prompt Preview -->
...
/**
 * Build the Loading Indicator subtab content.
 * @returns {string} HTML string for loading indicator subtab
 */
export function buildLoadingIndicatorSubtab() {
    const settings = extension_settings[LOADING_INDICATOR_MODULE] || defaultIndicatorSettings;
    return `
<!-- Enable/Disable -->
<div class="storymode-toggle">
<div class="storymode-toggle-info">
<span class="storymode-toggle-label">Show Loading Indicator</span>
<span class="storymode-toggle-description">Display themed loading messages during async operations</span>
</div>
<label class="storymode-switch">
<input type="checkbox" id="loading_indicator_enabled" ${settings.enabled ? 'checked' : ''}>
<span class="storymode-switch-slider"></span>
</label>
</div>
<!-- Position Settings -->
<div class="storymode-form-group">
<label class="storymode-form-label">Position</label>
<select id="loading_indicator_position" class="storymode-select">
...
<!-- Animation Style -->
<div class="storymode-form-group">
<label class="storymode-form-label">Animation Style</label>
<select id="loading_indicator_animation" class="storymode-select">
<option value="spin" ${settings.animationStyle === 'spin' ? 'selected' : ''}>Spin</option>
<option value="pulse" ${settings.animationStyle === 'pulse' ? 'selected' : ''}>Pulse</option>
<option value="bounce" ${settings.animationStyle === 'bounce' ? 'selected' : ''}>Bounce</option>
</select>
</div>
<!-- Custom GIF URL -->
<div class="storymode-form-group">
<label class="storymode-form-label">Custom GIF URL</label>
<input type="text" id="loading_indicator_gif_url" class="storymode-input" value="${settings.customGifUrl || ''}" placeholder="https://example.com/loading.gif">
<p class="storymode-form-hint">Leave empty to use CSS animation. Enter a GIF URL to use a custom loading image.</p>
</div>
<!-- Loading Messages -->
<div class="storymode-form-group">
<label class="storymode-form-label">Loading Messages</label>
<textarea id="loading_indicator_phrases" class="storymode-textarea" rows="6" placeholder="Enter one message per line">${(settings.phrases || []).join('\n')}</textarea>
<p class="storymode-form-hint">Enter one message per line. These will be randomly displayed during loading operations.</p>
</div>
<!-- Preview Button -->
<div class="storymode-form-group">
<button id="loading_indicator_preview" class="storymode-button" title="Test the loading indicator animation for 5 seconds">
<i class="fa-solid fa-play"></i> Preview Indicator (5 seconds)
</button>
</div>
`;
}
...
<button class="storymode-settings-subtab active" data-subtab="story_arc" title="Configure story arc settings">
<i class="fa-solid fa-book"></i> Story Arc
</button>
<button class="storymode-settings-subtab" data-subtab="author_style" title="Configure author style emulation">
<i class="fa-solid fa-pen-fancy"></i> Author Style
</button>
<button class="storymode-settings-subtab" data-subtab="blueprint_settings" title="Configure blueprint generation settings">
<i class="fa-solid fa-scroll"></i> Blueprint Settings
</button>
<button class="storymode-settings-subtab" data-subtab="post_arc" title="Configure auto-epilogue and summary options">
<i class="fa-solid fa-flag-checkered"></i> Post-Arc Options
</button>
<button class="storymode-settings-subtab" data-subtab="api_options" title="Configure API profiles and injection settings">
<i class="fa-solid fa-plug"></i> API Options
</button>
<button class="storymode-settings-subtab" data-subtab="loading_indicator" title="Configure loading indicator display">
<i class="fa-solid fa-spinner"></i> Loading Indicator
</button>
</div>
<!-- Settings Subtab Content Panes -->
<div id="settings_subtab_story_arc" class="storymode-settings-subtab-pane active">
${buildStoryArcSubtab()}
</div>
<div id="settings_subtab_author_style" class="storymode-settings-subtab-pane">
${buildAuthorStyleSubtab()}
</div>
<div id="settings_subtab_blueprint_settings" class="storymode-settings-subtab-pane">
${buildBlueprintSettingsSubtab()}
</div>
<div id="settings_subtab_post_arc" class="storymode-settings-subtab-pane">
${buildPostArcOptionsSubtab()}
</div>
<div id="settings_subtab_api_options" class="storymode-settings-subtab-pane">
${buildAPIOptionsSubtab()}
</div>
<div id="settings_subtab_loading_indicator" class="storymode-settings-subtab-pane">
${buildLoadingIndicatorSubtab()}
</div>
`;
}
...
<!-- Advanced: Edit Master Prompt -->
<div class="inline-drawer" id="blueprint_advanced_drawer">
<div class="inline-drawer-toggle inline-drawer-header" id="blueprint_advanced_header">
<span>Advanced: Edit Blueprint Generation Prompt</span>
<div class="inline-drawer-icon fa-solid fa-circle-chevron-down down interactable" tabindex="0" role="button"></div>
</div>
<div class="inline-drawer-content" id="blueprint_advanced_content">
<div class="storymode-advanced-content" id="blueprint_advanced_inner">
...
<!-- Action Buttons -->
<div class="storymode-generate-actions" style="margin-top: 24px;">
<button id="blueprint_generate_btn" class="menu_button storymode-btn storymode-btn-primary" title="Generate a story blueprint based on the settings above">
...
<button class="storymode-blueprint-subtab ${generateActive ? 'active' : ''}" data-subtab="generate" data-special="generate" title="Generate a new story blueprint">
<i class="fa-solid fa-wand-magic-sparkles"></i> Generate Blueprint
</button>
<button id="blueprint_import_btn" class="storymode-blueprint-subtab" data-subtab="load" title="Load a blueprint from a JSON file">
<i class="fa-solid fa-upload"></i> Load Blueprint
</button>
<span style="margin: 0 8px; color: var(--SmartThemeBorderColor); font-weight: 300;">|</span>
...
<button id="generate_opening_btn" class="menu_button storymode-btn storymode-btn-primary" title="Create the first scene message to start your story">
<i class="fa-solid fa-play"></i> Generate Opening Message
</button>
...
<input type="checkbox" id="save_opening_to_blueprint" style="cursor: pointer;" ${blueprint.opening_message ? 'checked' : ''}>
<span>Save to blueprint when generating</span>
</label>
</div>
</div>
` : `
...
<label style="display: flex; align-items: center; gap: 6px; cursor: pointer; font-size: 0.9em;">
<input type="checkbox" id="save_opening_to_blueprint" style="cursor: pointer;" checked>
<span>Save to blueprint when generating</span>
</label>
</div>
</div>
 ...
 * @returns {string} HTML string for summary tab
 */
export function buildSummaryTabContent() {
    const chatState = getChatStoryState();
    const storedSummary = chatState.lastSummary || null;
...
<p class="storymode-form-hint">Summaries are generated after arc completion if enabled in settings.</p>
</div>
`;
    }

    return `
<div class="storymode-summary-content">
${escapeHtml(storedSummary).replace(/\n/g, '<br>')}
</div>
`;
}
...
<div class="storymode-library-container">
    ...
    <div class="storymode-library-header">
        ...
        <div class="storymode-library-actions">
            <button id="library_save_current_btn" class="menu_button" title="Save current blueprint to library">
            ...
            <div class="storymode-view-toggle">
                <button id="library_view_grid" class="menu_button active" title="Grid view">
                    <i class="fa-solid fa-grid-2"></i>
                </button>
                <button id="library_view_list" class="menu_button" title="List view">
        ...
        <div class="storymode-library-main">
            <div class="storymode-library-grid" id="library_blueprint_grid">
                ...
                <div class="storymode-library-empty" id="library_empty_state">
                    ...
                    <p>Save your first blueprint to start building your library.</p>
                    <button id="library_empty_save_btn" class="menu_button storymode-btn storymode-btn-primary">
                        <i class="fa-solid fa-plus"></i> Save Current Blueprint
                    </button>
                </div>
            </div>
        </div>
