...
<!-- Action Buttons -->
        <div class="storymode-generate-actions" style="margin-top: 24px;">
            ...
            <button id="blueprint_cancel_generation_btn" class="menu_button storymode-btn storymode-btn-secondary" style="display: none;" title="Cancel the current blueprint generation">
                <i class="fa-solid fa-times"></i> Cancel Generation
            </button>
        </div>
    `;
}
...
export function buildWizardProgressHTML(currentPhase = 0) {
    const phases = [
        ...
        { number: 5, name: 'Validation', icon: 'fa-check-circle' },
    ];
...
function buildResolutionsPreview(bp) {
    ...
    return `
        <div class="storymode-wizard-preview-section">
            ...
            <p><strong>${escapeHtml(bp.primary_ending.title)}</strong></p>
            <p>${escapeHtml(bp.primary_ending.description)}</p>
        </div>
        `;
}
