...
import { buildPhaseOverridePanel } from './phase-override-panel.js';
import { pushStoryMessage } from '../../core/event-handlers.js';
import {
    buildChatCharacterPicker,
    buildPersonaPicker,
    buildLibraryCharacterPicker,
...
export function buildGenerateBlueprintSubtab() {
    ...
    // Get scenario from chat metadata or character description
    const context = getContext();
    const scenarioText = context.chatMetadata?.scenario || context.characterData?.description || '';
    const masterPrompt = getEffectiveMasterPrompt();

    // Build character pickers using unified component
    const chatChars = getCurrentChatCharacters();
    const chatCharIds = new Set(chatChars.map(c => c.id));
    const chatCharactersHtml = buildChatCharacterPicker(chatChars);
    const personasHtml = buildPersonaPicker(getAllPersonas());
    const libraryCharactersHtml = buildLibraryCharacterPicker(getAllCharacters(), chatCharIds);
    ...
    const selectedAuthorStyleId = chatState.selectedAuthorStyle || extension_settings[MODULE_NAME].defaultAuthorStyle || '';

    const storyTypes = getStoryTypes();
    const storyTypeOptions = [...storyTypes].sort((a, b) => a.name.localeCompare(b.name)).map(type =>
    ...
    const authorStyleOptions = ['<option value="">None</option>']
        .concat([...authorStyles].sort((a, b) => a.name.localeCompare(b.name)).map(style =>
            `<option value="${escapeHtml(style.id)}" ${style.id === selectedAuthorStyleId ? 'selected' : ''}>${escapeHtml(style.name)}</option>`
        ))
        .join('');

    // Get Connection Manager profiles and default profile for phase override panel
    ...
    const phaseOverridePanelHtml = buildPhaseOverridePanel(connectionProfiles, defaultProfileId);
    ...
    return `
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px; margin-bottom: 20px;">
            ...
            <div>
                <div class="storymode-form-group">
                    <label class="storymode-form-label">Character(s) in the current chat ${createHelpIcon('Characters from your active chat to include in the blueprint')}</label>
                    <div id="blueprint_characters_list" class="storymode-picker-container">${chatCharactersHtml}</div>
                </div>
                <div class="storymode-form-group">
                    <label class="storymode-form-label">Personas (Player Characters) ${createHelpIcon('Your player character identities to include')}</label>
                    <div id="blueprint_personas_list" class="storymode-picker-container">${personasHtml}</div>
                </div>
                <div class="storymode-form-group">
                    <label class="storymode-form-label">Add from Character Library ${createHelpIcon('Search and add additional characters from your full library')}</label>
                    <div id="blueprint_library_list_container" class="storymode-picker-container">${libraryCharactersHtml}</div>
                </div>
            </div>
        </div>
        ...
        <div class="storymode-generate-actions" style="margin-top: 24px; display: flex; flex-direction: row; justify-content: space-between; align-items: center;">
            ...
            <div style="flex: 1; display: flex; justify-content: flex-end;">
                <button id="blueprint_return_to_library_btn" class="menu_button storymode-btn storymode-btn-secondary" style="width: 100px; height: 35px; font-size: 0.85em;">Cancel</button>
            </div>
...
export function buildBlueprintTabContent() {
    ...
    const hasOpening = hasBlueprint && blueprint.opening_message;
    ...
    return `
        <div class="storymode-blueprint-subtabs">
            ...
            ${hasOpening ? '<button class="storymode-blueprint-subtab" data-subtab="opening"><i class="fa-solid fa-book-open"></i>&nbsp;Opening</button>' : ''}
...
export function renderBlueprintOverviewSubtab(blueprint, currentScene) {
    ...
    // Author style can be missing in two ways:
    // 1. Has an ID that doesn't exist in library
    // 2. Has no ID but has inline embedded data (author_style_name + author_style_prompt)
    const hasInlineAuthorStyle = !authorStyleId && blueprint.author_style_name && blueprint.author_style_prompt;
    const authorStyleMissing = (authorStyleId && isStyleMissing('authorStyle', authorStyleId)) || hasInlineAuthorStyle;

    const hasEmbeddedStoryType = storyTypeMissing && blueprint.embeddedResources?.storyType;
    const hasEmbeddedAuthorStyle = authorStyleMissing && (blueprint.embeddedResources?.authorStyle || hasInlineAuthorStyle);
    ...
    return `
        ...
        <div class="storymode-blueprint-overview-layout">
            ...
            ${blueprint.core_premise ? `<div class="storymode-blueprint-overview-text">${escapeHtml(blueprint.core_premise)}</div>` : ''}
        </div>
        <div class="storymode-blueprint-meta-row">
            <div class="storymode-info-card${storyTypeMissing ? ' storymode-missing-style' : ''}">
                ...
                ${storyTypeMissing ? `<button class="menu_button storymode-add-style-btn" data-style-type="storyType" data-style-id="${escapeHtml(storyTypeId)}" data-style-name="${escapeHtml(blueprint.story_type_name || '')}" data-has-embedded="${hasEmbeddedStoryType ? 'true' : 'false'}" title="${hasEmbeddedStoryType ? 'Import this story type' : 'Create this story type manually'}"><i class="fa-solid fa-plus"></i> Add</button>` : ''}
            </div>
            <div class="storymode-info-card${authorStyleMissing ? ' storymode-missing-style' : ''}">
                ...
                ${authorStyleMissing ? `<button class="menu_button storymode-add-style-btn" data-style-type="authorStyle" data-style-id="${escapeHtml(authorStyleId || '')}" data-style-name="${escapeHtml(blueprint.author_style_name || '')}" data-has-embedded="${hasEmbeddedAuthorStyle ? 'true' : 'false'}" data-has-inline="${hasInlineAuthorStyle ? 'true' : 'false'}" title="${hasEmbeddedAuthorStyle ? 'Import this author style to your library' : 'Create this author style manually'}"><i class="fa-solid fa-plus"></i> Add</button>` : ''}
            </div>
            <div class="storymode-info-card"><label>Total Scenes</label><span>${blueprint.scene_plan?.length || 0}</span></div>
            <div class="storymode-info-card"><label>Story Length</label><span>${BlueprintModule.calculateTotalRounds(blueprint)} rounds</span></div>
...
/**
 * Render the opening message subtab for blueprint
 */
export function renderBlueprintOpeningSubtab(blueprint) {
    if (!blueprint.opening_message) {
        return '<p class="storymode-form-hint">No opening message defined.</p>';
    }

    return `
        <div class="storymode-card">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                <h4 class="storymode-card-title" style="margin: 0;"><i class="fa-solid fa-book-open"></i> Opening Message</h4>
                <button id="inject_opening_message_btn" class="menu_button storymode-btn storymode-btn-primary" style="font-size: 0.8em; padding: 6px 16px;">
                    <i class="fa-solid fa-message"></i> Insert to Chat
                </button>
            </div>
            <div class="storymode-opening-message-content">${escapeHtml(blueprint.opening_message)}</div>
        </div>
    `;
}
...
export function renderBlueprintScenesSubtab(blueprint) {
    ...
    const scenesHtml = blueprint.scene_plan.map((scene, index) => {
        const sceneWithIndex = { ...scene, index };
        const beatsHtml = renderBeatProgress(sceneWithIndex);
        return `
            <div class="storymode-scenes-subtab-card">
                <div class="storymode-scenes-subtab-header">
                    <span class="storymode-scenes-subtab-number">${index + 1}</span>
                    <div class="storymode-scenes-subtab-title">${escapeHtml(scene.title || 'Untitled')}</div>
                    <span class="storymode-scenes-subtab-phase">${escapeHtml(scene.phase || 'Unknown')}</span>
                </div>
                ${scene.purpose ? `<p class="storymode-scenes-subtab-purpose">${escapeHtml(scene.purpose)}</p>` : ''}
                ${beatsHtml}
            </div>
        `;
    }).join('');

    return `<div class="storymode-scenes-subtab-list">${scenesHtml}</div>`;
}
