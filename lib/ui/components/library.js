            ...
            <div id="library_grid_view">
                ...
                <div class="storymode-library-header">
                    ...
                    <div class="storymode-library-actions">
                        ...
                        <button id="library_favorites_filter" class="menu_button interactable" data-active="false" title="Show favorites only" tabindex="0" role="button">
...
export function showLibraryGenerateView(content) {
    ...
    // Set up validation for custom scene count input
    setupSceneCountValidation(content);
}

/**
 * Set up validation for custom scene count input
 * Shows warnings for high scene counts and prevents invalid values
 * @param {jQuery} content - The settings dialog content element
 */
function setupSceneCountValidation(content) {
    const input = content.find('#blueprint_custom_rounds');
    const hint = content.find('#blueprint_custom_rounds_hint');
    const defaultHintText = 'Enter a custom number of scenes to override the selected length';
    const HIGH_SCENE_WARNING_THRESHOLD = 25;
    ...
    input.on('input change', function() {
        ...
        // Invalid: not a number or <= 0
        if (isNaN(parsed) || parsed <= 0) {
            hint.text('Scene count must be a number greater than 0');
            hint.css('color', 'var(--SmartThemeBodyColor)');
            return;
        }

        // Warning: high scene count
        if (parsed > HIGH_SCENE_WARNING_THRESHOLD) {
            hint.text('⚠️ High number of scenes may degrade generation and play through performance');
            hint.css('color', 'var(--SmartThemeEmColor)');
            return;
        }
