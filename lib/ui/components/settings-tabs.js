        ...
        <div class="storymode-overview-tab">
            ...
            <div class="storymode-section">
                ...
                <div class="storymode-quick-actions">
                    ...
                    <!-- Hidden: standalone editor not yet ready -->
                    <button id="open_fullscreen_editor_btn" class="menu_button storymode-quick-action storymode-quick-action-alt" title="Open the standalone blueprint editor in a new tab" style="display:none">
                        <i class="fa-solid fa-up-right-from-square"></i> Fullscreen Editor
                    </button>
                </div>
            </div>
        </div>
...
export function buildAPIOptionsSubtab() {
    // Load saved settings for injection options
    const settings = extension_settings[MODULE_NAME] || {};
    const savedPosition = settings.position ?? extension_prompt_types.IN_CHAT;
    const savedDepth = settings.depth ?? 4;
    const savedRole = settings.role ?? extension_prompt_roles.SYSTEM;
    const styleGenApi = settings.utilityApis?.styleGeneration || '';

    return `
<!-- Blueprint Generation APIs (existing) -->
...
<!-- Utilities APIs -->
<div class="storymode-card">
<h4 class="storymode-card-title">Utilities</h4>
<div class="storymode-form-group">
<label class="storymode-form-label">Style Generation Profile ${createHelpIcon('Which API connection to use for generating new author styles and story types')}</label>
<select id="style_generation_api" class="storymode-select storymode-api-select"
        data-setting-path="utilityApis.styleGeneration">
<option value="">Default API</option>
</select>
</div>
</div>
    ...
    <div class="inline-drawer-content" style="display: none;">
        ...
        'Before Prompt: Before anything else'
                ...
                <select id="injection_position" class="storymode-select">
                    <option value="${extension_prompt_types.IN_PROMPT}" ${savedPosition === extension_prompt_types.IN_PROMPT ? 'selected' : ''}>In Prompt</option>
                    <option value="${extension_prompt_types.IN_CHAT}" ${savedPosition === extension_prompt_types.IN_CHAT ? 'selected' : ''}>In Chat (at depth)</option>
                    <option value="${extension_prompt_types.BEFORE_PROMPT}" ${savedPosition === extension_prompt_types.BEFORE_PROMPT ? 'selected' : ''}>Before Prompt</option>
                </select>
            </div>
            <div class="storymode-form-group">
                ...
                <input type="number" id="injection_depth" class="storymode-input" min="0" max="100" value="${savedDepth}">
            </div>
            <div class="storymode-form-group">
        ...
        'User: As if from the player'
                ...
                <select id="injection_role" class="storymode-select">
                    <option value="${extension_prompt_roles.SYSTEM}" ${savedRole === extension_prompt_roles.SYSTEM ? 'selected' : ''}>System</option>
                    <option value="${extension_prompt_roles.ASSISTANT}" ${savedRole === extension_prompt_roles.ASSISTANT ? 'selected' : ''}>Assistant</option>
                    <option value="${extension_prompt_roles.USER}" ${savedRole === extension_prompt_roles.USER ? 'selected' : ''}>User</option>
                </select>
            </div>
            </div>
        </div>
