...
function renderPanelContent() {
    ...
    const currentStoryTypeId = settings.selectedStoryType || 'custom';
    const currentAuthorStyleId = settings.selectedAuthorStyle || '';

    const currentStoryType = storyTypes.find(st => st.id === currentStoryTypeId);
    const currentAuthorStyle = authorStyles.find(as => as.id === currentAuthorStyleId);
    ...
    if (currentStoryType || currentAuthorStyle) {
    ...
    // Arc History section - show if any arc completion content exists
    let arcHistoryInfo = '';
    if (chatState.savedEpilogue || chatState.savedSummary || chatState.savedNextAdventure) {
        ...
        if (hasNextAdventure) indicators.push('<i class="fa-solid fa-route" title="Next Adventure"></i>');
        ...
        arcHistoryInfo = `
            <div id="storymode-debug-arc-history-link" class="storymode-debug-section storymode-debug-interactable" title="Click to view arc completion content">
                <div class="storymode-debug-label">Arc History <i class="fa-solid fa-magnifying-glass"></i></div>
                <div class="storymode-debug-value">${indicators.join(' ')}</div>
                <div class="storymode-debug-meta">${[hasEpilogue && 'Epilogue', hasSummary && 'Summary', hasNextAdventure && 'Next Adventure'].filter(Boolean).join(', ')}</div>
            </div>
        `;
    }
...
function showArcHistoryPopup() {
    ...
    // Epilogue section
    if (chatState.savedEpilogue) {
        const timestamp = chatState.savedEpilogue.timestamp
            ? new Date(chatState.savedEpilogue.timestamp).toLocaleString()
            : 'Unknown';
        sectionsHtml += `
            <div class="storymode-arc-history-section">
                <div class="storymode-arc-history-header">
                    <h4><i class="fa-solid fa-scroll"></i> Epilogue</h4>
                    <span class="storymode-arc-history-timestamp">${escapeHtml(timestamp)}</span>
                </div>
                <div class="storymode-arc-history-content">
                    ${escapeHtml(chatState.savedEpilogue.text)}
                </div>
            </div>
        `;
    }
    ...
    // Summary section
    if (chatState.savedSummary) {
        const timestamp = chatState.savedSummary.timestamp
            ? new Date(chatState.savedSummary.timestamp).toLocaleString()
            : 'Unknown';
        sectionsHtml += `
            <div class="storymode-arc-history-section">
                <div class="storymode-arc-history-header">
                    <h4><i class="fa-solid fa-file-lines"></i> Summary</h4>
                    <span class="storymode-arc-history-timestamp">${escapeHtml(timestamp)}</span>
                </div>
                <div class="storymode-arc-history-content">
                    ${escapeHtml(chatState.savedSummary.text)}
                </div>
            </div>
        `;
    }
    ...
    if (chatState.savedNextAdventure) {
        const timestamp = chatState.savedNextAdventure.timestamp
            ? new Date(chatState.savedNextAdventure.timestamp).toLocaleString()
            : 'Unknown';

        // Themes subsection
        let themesHtml = '';
        if (chatState.savedNextAdventure.themes && chatState.savedNextAdventure.themes.length > 0) {
            themesHtml = `
                <div class="storymode-next-adventure-subsection">
                    ...
                    <ul class="storymode-theme-list">
                        ${chatState.savedNextAdventure.themes.map(theme => `
                            <li>
        ...
        let scenariosHtml = '';
        if (chatState.savedNextAdventure.scenarios && chatState.savedNextAdventure.scenarios.length > 0) {
            scenariosHtml = `
                <div class="storymode-next-adventure-subsection">
                    <h5><i class="fa-solid fa-masks-theater"></i> Scenario Options</h5>
                    <div class="storymode-scenario-cards">
                        ${chatState.savedNextAdventure.scenarios.map((scenario, idx) => `
                            <div class="storymode-next-adventure-option">
                                ...
                                ${scenario.hooks && scenario.hooks.length > 0 ? `
        ...
        sectionsHtml += `
            <div class="storymode-arc-history-section">
                <div class="storymode-arc-history-header">
                    <h4><i class="fa-solid fa-route"></i> Next Adventure</h4>
                    <span class="storymode-arc-history-timestamp">${escapeHtml(timestamp)}</span>
                </div>
                <div class="storymode-arc-history-content">
                    ${themesHtml}
