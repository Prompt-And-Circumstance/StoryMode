...
function renderPanelContent() {
    ...
    // Determine what to show based on settings and pacing mode (#3 - flattened)
    // Show story type if: exists AND (in Scenario Mode OR storyTypeEnabled in Story Mode)
...
function setupRpgCompanionCoexistence() {
    ...
    // Cleanup any existing observer before creating a new one (prevents accumulation)
    if (window._storyModeRpgObserver) {
        window._storyModeRpgObserver.disconnect();
    }
