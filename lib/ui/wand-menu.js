...
export function getWandMenuStatusText() {
    ...
    let authorDisplay = '';
    if (settings.authorStyleEnabled) {
        const { styleId, source, sourceName } = getEffectiveAuthorStyle();
        if (styleId) {
            const authorStyle = authorStyles.find(s => s.id === styleId);
            if (authorStyle) {
                authorDisplay = authorStyle.name;
                // Show source indicator for overrides
                if (source === 'blueprint' && sourceName) {
                    authorDisplay += ` (from ${sourceName})`;
                } else if (source === 'character' && sourceName) {
                    authorDisplay += ` (from ${sourceName})`;
                } else if (source === 'group' && sourceName) {
                    authorDisplay += ` (from ${sourceName})`;
                }
                // 'chat' and 'default' don't need indicators
            }
        }
    }
    ...
    } else if (settings.storyArcEnabled) {
        // Only show round count when arc mode is enabled
    ...
    if (authorDisplay) {
        status += status ? `\nAuthor: ${authorDisplay}` : `Author: ${authorDisplay}`;
    }
...
export function updateDropdownContent() {
    ...
    const pacingMode = getPacingMode();
    const isStoryMode = pacingMode === PACING_MODES.STORY;
    ...
    // Show/hide arc control buttons (only in Story Mode with story type selected)
    // In Scenario Mode, progression is signal-based via beats/scenes, not round-based
    const showArcControls = isEnabled && hasStoryType && isStoryMode;
    dropdown.find('#wand_step_forward').toggle(showArcControls);
    dropdown.find('#wand_step_back').toggle(showArcControls);
    dropdown.find('#wand_reset_arc').toggle(showArcControls);
    ...
    const arcControlsVisible = showArcControls;
...
/**
 * Show loading dialog for generation tasks
 * @param {string} message - Loading message to display
 * @returns {Popup} Popup instance (call .complete() to close)
 */
function showLoadingDialog(message) {
    const html = `
        <div style="text-align: center; padding: 30px 20px;">
            <div style="margin-bottom: 20px;">
                <i class="fa-solid fa-circle-notch fa-spin" style="font-size: 48px; color: var(--SmartThemeEmColor);"></i>
            </div>
            <h3 style="margin: 0 0 12px 0; text-align: center; font-size: 1.3em; border-left: none;">${message}</h3>
            <p style="color: var(--SmartThemeQuoteColor); margin: 0; text-align: center;">This may take up to a minute, or two...</p>
        </div>
    `;
 ...
 */
async function confirmRegeneration(taskName, shownFlag) {
    console.log(`[Story Mode] confirmRegeneration: taskName=${taskName}, shownFlag=${shownFlag}`);
    ...
    const messages = {
        ...
        nextAdventure: "What's Next options have already been generated. Generate new options?"
    };
    const result = await callGenericPopup(
        messages[taskName] || 'Already generated. Generate again?',
        POPUP_TYPE.CONFIRM,
        '',
        { okButton: 'Generate Again', cancelButton: 'Cancel' }
    );
    console.log(`[Story Mode] confirmRegeneration result: ${result} (AFFIRMATIVE=${POPUP_RESULT.AFFIRMATIVE})`);
...
function saveGeneratedContent(chatState, taskName, result) {
    ...
    if (taskName === 'epilogue') {
        chatState.savedEpilogue = { text: result, timestamp };
    } else if (taskName === 'summary') {
...
async function generateContent(taskName, generatorFn, chatState, stateKey, formatFn = null) {
    if (!await confirmRegeneration(taskName, chatState[stateKey])) return;

    // Map task names to display names
    const displayNames = {
        'epilogue': 'Epilogue',
        'summary': 'Summary',
        'nextAdventure': "What's Next"
    };

    const taskDisplayName = displayNames[taskName] || taskName.charAt(0).toUpperCase() + taskName.slice(1);
    const loadingMessage = `Generating ${taskDisplayName}`;

    // Show loading dialog and track if user cancels
    const loadingPopup = showLoadingDialog(loadingMessage);
    ...
    try {
        ...
        if (userCancelled) {
            toastr.info(`${taskDisplayName} generation cancelled`, 'Story Mode');
            return;
        }
        ...
        toastr.success(`${taskDisplayName} generated successfully!`, 'Story Mode');
    } catch (error) {
        ...
        if (!userCancelled) {
            loadingPopup.complete(POPUP_RESULT.AFFIRMATIVE);
        }
    }
}
...
export async function generateSummary() {
    console.log('[Story Mode] generateSummary called');
    const chatState = getChatStoryState();
    console.log('[Story Mode] chatState.summaryShown:', chatState.summaryShown);
...
export async function registerWandMenuEntry() {
        ...
        <div id="story_mode_dropdown">
            <ul class="list-group">
                ...
                <li class="list-group-item" id="wand_generate_next_adventure" title="Generate scenario options for continuing the story">
                    <i class="fa-solid fa-route"></i> What's Next
                </li>
    ...
    dropdown.find('#wand_generate_summary').on('click', async function () {
        console.log('[Story Mode] Generate Summary clicked');
        try {
            await generateSummary();
            updateWandMenuStatus();
            dropdown.fadeOut(animation_duration);
        } catch (error) {
            console.error('[Story Mode] Generate Summary error:', error);
            toastr.error(`Failed to generate summary: ${error.message}`);
        }
    });
