 ...
 * This can be called manually from the wand menu at any time
 *
 * @async
 * @returns {Promise<void>}
 */
export async function generateEpilogue() {
    ...
    // Check if epilogue was already shown
    if (chatState.epilogueShown) {
        if (!confirm('An epilogue has already been generated for this arc. Generate another?')) {
            return;
        }
    }
    ...
    try {
        ...
        if (!epilogue || !epilogue.trim()) {
            toastr.error('Failed to generate epilogue: No content generated', 'Story Mode');
            return;
        }

        // Push the epilogue to chat
        ...
        toastr.success('Epilogue generated successfully!', 'Story Mode');
    } catch (error) {
        console.error('[Story Mode] Error generating epilogue:', error);
        toastr.error(`Failed to generate epilogue: ${error.message}`, 'Story Mode');
    } finally {
    }
}
...
export async function generateSummary() {
    ...
    // Check if summary was already shown
    if (chatState.summaryShown) {
        if (!confirm('A summary has already been generated for this arc. Generate another?')) {
            return;
        }
    }
    ...
    try {
        ...
        if (!summary || !summary.trim()) {
            toastr.error('Failed to generate summary: No content generated', 'Story Mode');
            return;
        }
        ...
        toastr.success('Summary generated successfully!', 'Story Mode');
    } catch (error) {
        console.error('[Story Mode] Error generating summary:', error);
        toastr.error(`Failed to generate summary: ${error.message}`, 'Story Mode');
    }
}
...
export async function generateNextAdventure() {
    ...
    // Check if next adventure was already shown
    if (chatState.nextAdventureShown) {
        if (!confirm('Next adventure options have already been generated. Generate new options?')) {
            return;
        }
    }

    try {
        // Generate the next adventure options
        const result = await generateNextAdventureForStory();

        if (!result || !result.scenarios?.length) {
            toastr.error('Failed to generate next adventure options', 'Story Mode');
            return;
        }
        ...
        toastr.success('Next adventure options generated!', 'Story Mode');
    } catch (error) {
        console.error('[Story Mode] Error generating next adventure:', error);
        toastr.error(`Failed to generate next adventure: ${error.message}`, 'Story Mode');
    }
}
