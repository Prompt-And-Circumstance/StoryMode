...
export function getWandMenuStatusText() {
    ...
    let status = `Round: ${chatState.currentStep}/${chatState.arcLength}`;
    if (storyName !== 'None') {
        status += `\nStory: ${storyName}`;
    }
    if (authorName) {
...
export async function registerWandMenuEntry() {
    ...
    if ($('#story_mode_wand_button').length > 0) {
        console.log('[Story Mode] Wand menu item already registered');
        return;
    }
    ...
    const dropdownHtml = `
        <div id="story_mode_dropdown">
            <ul class="list-group">
                ...
                <li class="list-group-item" id="wand_disable" title="Turn off Story Mode for this chat">
                    <i class="fa-solid fa-power-off"></i> Disable Story Mode
                </li>
    ...
    console.log('[Story Mode] Wand menu item registered');
}
